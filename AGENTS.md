...
- The accidental scaffold is the sibling checkout named `sundog-vite`; the real
  website source is this repo root.
...
The deploy helper uses `CLOUDFLARE_API_TOKEN` when it is already set in the
environment; otherwise it relies on the operator's local non-repo Cloudflare
credential store. The store path can be configured with
`SUNDOG_CLOUDFLARE_ENV_FILE`; otherwise the helper uses a generic hidden file in
the operator's home directory. Do not document, print, commit, or copy local
secret paths or secret values into this repo.
...
- Binary: `%USERPROFILE%\HalSim361.exe` — a 2004 GUI app with **no CLI
  and no headless mode**. The `.sim` / `.xsh` / `.xng` / `.xmt` asset
  library and the `h*.txt` help corpus are in `%USERPROFILE%\`.
...
1. Copy the target `.sim` over `%USERPROFILE%\Startup.sim` (HaloSim's
...
4. Harvest `%USERPROFILE%\autosave.bmp` (auto-written on completion;
...
- PyMacroRecord (`%USERPROFILE%\PyMacroRecord`) was evaluated as the
...
Cloudflare credentials are local operator secrets. Keep credential files and
secret values outside the repo, and do not document local secret paths in repo
files. Use `SUNDOG_CLOUDFLARE_ENV_FILE` for the scoped-token store and
`SUNDOG_CLOUDFLARE_LEGACY_KEY_FILE` only for a break-glass legacy credential
file.
...
Those commands verify the configured Cloudflare credential shape without
printing secrets.
...
directory `dist`. Any Cloudflare IDs, token IDs, or one-time token values belong
only in the local non-repo credential store.

`npm run cf:tokens` creates scoped Cloudflare API tokens for Pages deploy,
Workers editing, DNS editing, and readonly session-agent inspection. Prefer
those scoped tokens over any break-glass owner credential for ordinary
automation.
