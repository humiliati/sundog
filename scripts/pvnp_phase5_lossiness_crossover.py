...
# S0
S0 = dict(T=64, w=0.5, w_b=0.1, t0=0.0, A=1.0, C=1.0, D=0.5, f_p=8.0, xc_lo=3.0, xc_hi=7.0)
# S1
...
def gen_s0(n, lam, rng, noise):
    ...
    env = np.exp(-t ** 2 / (2 * c["w"] ** 2))
    bump = np.exp(-(t - c["t0"]) ** 2 / (2 * c["w_b"] ** 2))
    xc = rng.uniform(c["xc_lo"], c["xc_hi"], n)                       # continuous label
    xd = rng.choice([-1.0, 1.0], n)                                  # discrete label
    ...
    sig = c["D"] * bump[None, :] + c["A"] * fringe * env[None, :] + c["C"] * parity * env[None, :]
...
def main():
    ...
    t0 = time.time()
    print(f"[cfg] mode={mode} seed={seed} n={n} noise={args.noise} K={K} CV={CV} "
          f"lambdas={LAMBDAS}", flush=True)

    print("S0 (1-D caustic toy):", flush=True)
    r0 = sweep(gen_s0, n, seed, args.noise, "S0")
    print("S1 (2-D vector field):", flush=True)
    r1 = sweep(gen_s1, n, seed, args.noise, "S1")
    ...
    (out / f"{mode}.json").write_text(json.dumps(
        {"mode": mode, "seed": seed, "n": n, "noise": args.noise, "lambdas": LAMBDAS,
