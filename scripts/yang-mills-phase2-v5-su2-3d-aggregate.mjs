#!/usr/bin/env node
// scripts/yang-mills-phase2-v5-su2-3d-aggregate.mjs
//
// Yang-Mills Phase 2 v5 - symmetric Polyakov powered-target audit followed, only if admitted,
...
const LOCKED = Object.freeze({
  ...
  outRequired: "results/yang-mills/phase2/SU2_3D/2026-05-31_su2_3d_relative_locality_v5",
  powerIccGate: 0.50,
  ...
  gaugeRandSeedTag: "phase2_v5_aggregation",
...
const POLYAKOV_SAMPLE_COUNT = 12 * 12 * 3;
...
function compareV0V5(scoreRows, primaryTarget) {
  ...
  for (const row of scoreRows) {
    ...
    rows.push({
      ...
      v5PrimaryTarget: primaryTarget,
      v5MeanBinPurity: v5Mean,
      deltaV5MinusV0: v0Mean === undefined ? null : v5Mean - v0Mean,
    });
  }
...
function assessStage2Branch(inputs) {
  ...
  if (!inputs.v5BinEdgeTimestampOk) return ["Z void_run", "v5 bin-edge timestamp drift"];
  ...
  return ["P2-A bounded_positive", "every Phase 2 v5 point-estimate gate passes on a powered Polyakov target"];
}
...
function stage1Branch(admittedTarget, gammaAudit, polyakovGaugeResidualMax) {
  ...
  if (gammaAudit.poweredAllBetas) {
    return ["Z void_run", "gamma_held passed the v5 power self-validation gate"];
  }
...
function computePolyakovGaugeHealth(records) {
  ...
  for (const r of records) {
    ...
    const gaugePolyakov = computePolyakovV4(transformed);
...
async function main() {
  ...
  if (failures.length > 0) {
    console.error("[YM-P2-V5-AGG] manifest validation FAILED:");
  ...
  ensureCleanDir(aggDir, "results/yang-mills/phase2/SU2_3D/2026-05-31_su2_3d_relative_locality_v5/aggregation");
  ...
  console.log("[YM-P2-V5-AGG] Stage 1: computing transverse-parity Polyakov summaries for powered-target audit");
  const targetRows = [];
  for (const r of records) {
    r.v4Loops = computeV4LoopSummaries(r.config);
    r.polyakov = computePolyakovV4(r.config);
    const values = {};
  ...
  writeCSV(
    path.join(aggDir, "v5_polyakov_target_resummaries.csv"),
    [
      "beta",
      "config_idx",
      "abs_mean_P",
  ...
  if (stage1OnlyBranch) {
    ...
    console.log(`[YM-P2-V5-AGG] verdict: ${stage1OnlyBranch}`);
    console.log(`[YM-P2-V5-AGG] ${stage1OnlyReason}`);
    console.log(`[YM-P2-V5-AGG] wall clock ${wallClockSeconds.toFixed(2)} s`);
    return;
  }
  ...
  const perBetaEdgesPath = path.join(aggDir, "per_beta_v5_bin_edges.json");
  const globalEdgesPath = path.join(aggDir, "global_v5_bin_edges.json");
  ...
  console.log(`[YM-P2-V5-AGG] Stage 2: scoring admitted powered Polyakov target ${primaryTarget}`);
  ...
  writeJSON(path.join(aggDir, "v0_v5_comparison.json"), compareV0V5(scoreRows, primaryTarget));
  ...
  const branchInputs = {
    ...
    v5BinEdgeTimestampOk: edgeMtimeMs < firstScoringArtifactMtimeMs,
    binDegenerateBetas,
  ...
  const summary = {
    ...
    integrity: {
      ...
      v5BinEdgeTimestampOk: branchInputs.v5BinEdgeTimestampOk,
      gaugeSignatureResidualMax,
      gaugeTargetResidualMax,
      polyakovGaugeResidualMax: polyakovGaugeHealth.polyakovGaugeResidualMax,
      polyakovGaugeResidualByTarget: polyakovGaugeHealth.perTargetMaxResidual,
    ...
    target: {
      primaryTarget,
      perBetaBinEdgesPath: "aggregation/per_beta_v5_bin_edges.json",
      globalBinEdgesPath: "aggregation/global_v5_bin_edges.json",
      binDegenerateBetas,
    },
    wallClockSeconds,
  };
  ...
  console.log(`[YM-P2-V5-AGG] verdict: ${branch}`);
  console.log(`[YM-P2-V5-AGG] ${reason}`);
  console.log(`[YM-P2-V5-AGG] admitted target ${primaryTarget}; mean ICC ${admittedTarget.primary.meanIcc.toFixed(4)}; mean leakage ${admittedTarget.primary.meanLeakageCvR2.toFixed(4)}`);
  console.log(`[YM-P2-V5-AGG] within primary@5 ${branchInputs.withinPrimary5.toFixed(4)}; raw ${branchInputs.withinCtrlRaw5.toFixed(4)}; rand ${branchInputs.withinCtrlRand5.toFixed(4)}`);
  console.log(`[YM-P2-V5-AGG] gauge residuals signature=${gaugeSignatureResidualMax.toExponential(3)} target=${gaugeTargetResidualMax.toExponential(3)}`);
  console.log(`[YM-P2-V5-AGG] wall clock ${wallClockSeconds.toFixed(2)} s`);
}

main().catch((err) => {
  console.error("[YM-P2-V5-AGG] runner threw:");
