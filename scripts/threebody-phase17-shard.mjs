// scripts/threebody-phase17-shard.mjs
//
// Parameterized wrapper for one Phase 17 hazard-aligned-counterfactual lock
// shard, per PHASE17_SPEC.md (same 12-shard partition as Phase 15C: by
// mass-ratio × velocityScale, each keeping all 6 modes + 3 radii + 8 seeds).
//
// Usage:
//   node scripts/threebody-phase17-shard.mjs --mass-ratio <X> --velocity-scale <Y>
//   (or via npm:  npm run threebody:phase17:shard -- --mass-ratio 1 --velocity-scale 0.95)
//
// Pinned values:
...
//
// All other flags are pinned to the Phase 15B/15C lock slate plus
// --hazard-counterfactual-audit 1 (no --precision-receipts: Phase 17 drops the
// oracle; no --track-action-coupling: it does not change dynamics or the
// candidate-envelope classification).
...
function parseArgs(argv) {
  ...
  if (!ALLOWED_MASS_RATIOS.has(args.massRatio)) {
    throw new Error(`--mass-ratio must be one of ${[...ALLOWED_MASS_RATIOS].join(", ")} (got "${args.massRatio}"); per PHASE17_SPEC.md`);
  }
  if (!ALLOWED_VELOCITY_SCALES.has(args.velocityScale)) {
    throw new Error(`--velocity-scale must be one of ${[...ALLOWED_VELOCITY_SCALES].join(", ")} (got "${args.velocityScale}"); per PHASE17_SPEC.md`);
  }
  return args;
}
...
function buildHarnessArgs(massRatio, velocityScale) {
  ...
  return [
    ...
    "--phase", `phase17-hazard-counterfactual-shard-${tag}`,
    "--out",   `results/threebody/phase17-shard-${tag}`,
    "--regimes", "near_escape",
    "--modes",
...
function run() {
  ...
  console.log(`[shard ${tag}] out: results/threebody/phase17-shard-${tag}`);
