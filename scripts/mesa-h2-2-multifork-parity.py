"""Check Python H2.2 multi-fork env parity against JS-generated fixtures."""
...
from training.mesa.h2_multifork_task import MultiForkEnv, make_controller


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser()
    ap.add_argument("--fixtures", type=Path, default=Path("results/mesa/h2-frontier/h2_2_parity/fixtures.json"))
...
def main() -> int:
    ...
    for episode in payload["episodes"]:
        ...
        if env.key != episode["initialKey"]:
            raise AssertionError(
                f"hidden key mismatch for {episode['cell']} seed {episode['seed']}: "
                f"{env.key} != {episode['initialKey']}"
            )
        ...
        for i, row in enumerate(episode["trace"]):
            ...
            max_diff = max(max_diff, assert_close(f"{prefix}:x", [env.x, env.y], row["x"], args.tol))
            if env.phase != row["phase"]:
                raise AssertionError(f"{prefix}: phase mismatch {env.phase} != {row['phase']}")
            ...
            max_diff = max(max_diff, assert_close(f"{prefix}:rewardMagnitude", env.reward_magnitude(), row["rewardMagnitude"], args.tol))
            action = ctrl.act(env, obs)
            max_diff = max(max_diff, assert_close(f"{prefix}:action", action, row["action"], args.tol))
            step = env.step(action)
            max_diff = max(max_diff, assert_close(f"{prefix}:afterX", [env.x, env.y], row["afterX"], args.tol))
    ...
    print(
        f"H2.2 multi-fork parity PASS: episodes={len(payload['episodes'])} steps={step_count} "
