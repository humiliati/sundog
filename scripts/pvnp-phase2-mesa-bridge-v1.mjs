#!/usr/bin/env node
// Phase 2 mesa-bridge v1 harness.
//
// Reads canonical mesa Phase 4 raw trial logs plus the v1 repaired Small raw
// logs, recomputes reward-blind response summaries, verifies a small bridge
// certificate, and scores it against the frozen v1 slate labels. It does not
// rerun mesa training or simulation.
...
const DEFAULT_OUT = V1_OUT;
const PHASE4_REPAIRED_ROOT = V1_SMALL_RERUN_ROOT;
const PHASE4_CANONICAL_ROOT = "results/mesa/phase4-intervention-battery";
...
const SCHEMA = "pvnp-phase2-mesa-bridge-sigma-v1";
const TRANSFORM_ID = "H_mesa_bridge_v1_provenance_repair";
...
const RAW_LOG_CHANNELS = Object.freeze(["reward", "observation", "signature-sensor", "geometry", "basin-position"]);
const SMALL_RAW_SLUGS = new Set(SMALL_RAW_BATTERIES.map((battery) => battery.slug));
...
async function pathExists(relPath) {
  try {
    await access(path.resolve(REPO_ROOT, relPath));
    return true;
  } catch {
    return false;
  }
...
function expectedTrialFiles(seedBase, seedCount, channels) {
  const files = [];
  for (let offset = 0; offset < seedCount; offset += 1) {
    const seed = seedBase + offset;
    for (const channel of channels) {
      files.push(`trials/${seed}-${channel}-off.jsonl`);
      files.push(`trials/${seed}-${channel}-on.jsonl`);
    }
  }
  return files;
}

async function inspectPolicyCandidate(root, policySlug) {
  const base = `${root}/${policySlug}`;
  const dirExists = await pathExists(base);
  if (!dirExists) {
    return {
      root,
      base,
      ...
      trial_logs_saved: false,
      seed_base: null,
      seed_count: null,
      required_trial_files: 0,
      missing_trial_files: null,
  ...
  try {
    manifestArtifact = await readTextWithHash(`${base}/manifest.json`);
  } catch (err) {
    return {
      root,
      base,
      ...
      missing_trial_files: null,
      complete_for_selection: false,
      reason: `manifest missing: ${err.message}`,
    };
  }
  ...
  const expected = Number.isFinite(seedBase) && Number.isFinite(seedCount)
    ? expectedTrialFiles(seedBase, seedCount, RAW_LOG_CHANNELS)
    : [];
  let missing = 0;
  for (const relFile of expected) {
    if (!(await pathExists(`${base}/${relFile}`))) missing += 1;
  }
  const trialLogsSaved = manifest.trial_logs_saved === true;
  const complete = trialLogsSaved && expected.length > 0 && missing === 0;
  return {
    root,
    base,
    policySlug,
    ...
    manifest_bytes: manifestArtifact.bytes,
    trial_logs_saved: trialLogsSaved,
    seed_base: seedBase,
    seed_count: seedCount,
    required_trial_files: expected.length,
    missing_trial_files: missing,
    complete_for_selection: complete,
    reason: complete ? "complete raw-log policy directory" : "missing or disabled raw logs",
    manifest,
  };
}

async function resolvePolicyInput(policySlug, inputResolutionRows, voidReasons) {
  const repaired = await inspectPolicyCandidate(PHASE4_REPAIRED_ROOT, policySlug);
  const canonical = await inspectPolicyCandidate(PHASE4_CANONICAL_ROOT, policySlug);
  let selected = canonical;
  let selectedReason = "canonical root selected";

  if (repaired.directory_exists) {
    selected = repaired;
    selectedReason = repaired.complete_for_selection
      ? "repaired root selected"
      : "repaired root present but invalid; fallback forbidden";
    if (!repaired.complete_for_selection) {
      voidReasons.push(`${policySlug}: repaired root present but incomplete (${repaired.reason})`);
    }
  } else if (!canonical.directory_exists) {
    voidReasons.push(`${policySlug}: no phase4 policy directory found`);
  }

  inputResolutionRows.push({
    policySlug,
    selected_root: selected.root,
    selected_base: selected.base,
    selected_reason: selectedReason,
    selected_manifest_exists: selected.manifest_exists ? 1 : 0,
    selected_trial_logs_saved: selected.trial_logs_saved ? 1 : 0,
    selected_required_trial_files: selected.required_trial_files,
    selected_missing_trial_files: selected.missing_trial_files,
    repaired_root_exists: repaired.directory_exists ? 1 : 0,
    repaired_complete_for_selection: repaired.complete_for_selection ? 1 : 0,
    canonical_root_exists: canonical.directory_exists ? 1 : 0,
    canonical_complete_for_selection: canonical.complete_for_selection ? 1 : 0,
    is_small_raw_repair_slug: SMALL_RAW_SLUGS.has(policySlug) ? 1 : 0,
  });
...
async function loadPolicyRaw(policySlug, opCounter, sourceHashes, inputResolutionRows, voidReasons) {
  const selected = await resolvePolicyInput(policySlug, inputResolutionRows, voidReasons);
  const base = selected.base;
  if (selected.manifest_exists) {
    sourceHashes.push({
      path: `${base}/manifest.json`,
      sha256: selected.manifest_sha256,
      bytes: selected.manifest_bytes,
  ...
  const integrityRows = [];
  let missingPairs = 0;
  let rawFilesRead = selected.manifest_exists ? 1 : 0;
  let rawBytesRead = selected.manifest_bytes ?? 0;
  ...
  if (manifest.trial_logs_saved !== true) {
    for (const channel of REQUIRED_CHANNELS) {
      ...
      integrityRows.push({
        ...
        passed: 0,
        detail: selected.manifest_exists
          ? "manifest does not record trial_logs_saved=true; raw recomputation unavailable"
          : "manifest missing; raw recomputation unavailable",
      });
    }
    return {
      policySlug,
...
function makeCertificate(cell, raw) {
  ...
  return {
    ...
    access_profile: "phase2-mesa-bridge-v1-reward-blind",
...
function verdict(labels, decisions, audits, integrityRows, rawRecomputeRows, opReport, populationRows, inputResolutionRows, voidReasons) {
  const dByCell = new Map(decisions.map((d) => [d.cell_id, d]));
  ...
  const smallRawRows = inputResolutionRows.filter((row) => Number(row.is_small_raw_repair_slug) === 1);
  const smallRawRegenerated = smallRawRows.length === SMALL_RAW_SLUGS.size
    && smallRawRows.every((row) => (
      row.selected_root === PHASE4_REPAIRED_ROOT
      && Number(row.selected_trial_logs_saved) === 1
      && Number(row.selected_missing_trial_files) === 0
    ));
  const populationPreserved = populationRows.length === CELLS.length
    && populationRows.every((row) => Number(row.population_preserved) === 1);
  const gates = {
    small_raw_logs_regenerated: smallRawRegenerated,
  ...
  let runVerdict = boundedPositiveEligible ? "bounded_positive_eligible" : "named_quarantine";
  if (voidReasons.length > 0) {
    runVerdict = "void_run";
  } else if (fixedFalseAccepts > 0 || capacityFalseAccepts > 0 || mixedLaundering > 0) {
    runVerdict = "falsified_registered_cell";
  }
  return {
    verdict: runVerdict,
    gates,
    ...
    counts: {
      ...
      small_raw_repair_slugs_resolved: smallRawRows.length,
      population_rows: populationRows.length,
    },
  };
}
...
function populationAuditRows() {
  return CELLS.map((cell, index) => ({
    ordinal: index + 1,
    v0_cell_id: cell.cell_id,
    v1_cell_id: cell.cell_id,
    v0_policySlug: cell.policySlug,
    v1_policySlug: cell.policySlug,
    v0_role: cell.role,
    v1_role: cell.role,
    population_preserved: 1,
  }));
}

async function collectSmallRawRerunManifest(inputResolutionRows) {
  const rowsBySlug = new Map(inputResolutionRows.map((row) => [row.policySlug, row]));
  let priorRunsBySlug = new Map();
  let prior_manifest_sha256 = "";
  try {
    const priorArtifact = await readTextWithHash(`${V1_OUT}/small_raw_rerun_manifest.json`);
    const prior = JSON.parse(priorArtifact.text);
    prior_manifest_sha256 = priorArtifact.sha256;
    priorRunsBySlug = new Map((prior.runs ?? []).map((run) => [run.slug, run]));
  } catch {
    // No prior runner manifest exists when the bridge is invoked before the
    // post-freeze raw-log repair command.
  }
  const runs = [];
  for (const battery of SMALL_RAW_BATTERIES) {
    const resolution = rowsBySlug.get(battery.slug);
    const priorRun = priorRunsBySlug.get(battery.slug);
    const manifestPath = `${V1_SMALL_RERUN_ROOT}/${battery.slug}/manifest.json`;
    let manifest = null;
    let manifestSha256 = null;
    let manifestBytes = 0;
    try {
      const artifact = await readTextWithHash(manifestPath);
      manifest = JSON.parse(artifact.text);
      manifestSha256 = artifact.sha256;
      manifestBytes = artifact.bytes;
    } catch {
      // The bridge run may be invoked before the raw-log repair runner; the
      // pass gates will fail rather than silently falling back.
    }
    runs.push({
      slug: battery.slug,
      label: battery.label,
      command: powerShellCommandForBattery(battery),
      elapsed_ms: priorRun?.elapsed_ms ?? "",
      ...
      selected_by_bridge: resolution?.selected_root === V1_SMALL_RERUN_ROOT ? 1 : 0,
      missing_trial_files: resolution?.selected_missing_trial_files ?? "",
    });
  }
  return {
    schema: "pvnp-phase2-mesa-bridge-v1-small-raw-rerun-manifest",
    run_id: `${V1_RUN_ID}:small-raw-readback`,
    output_root: V1_SMALL_RERUN_ROOT,
    command_file: `${V1_OUT}/small_raw_rerun_commands.ps1`,
    prior_runner_manifest_sha256: prior_manifest_sha256,
    run_count: runs.length,
    all_trial_logs_saved: runs.every((run) => run.trial_logs_saved),
    all_selected_by_bridge: runs.every((run) => run.selected_by_bridge === 1),
    runs,
  };
}
...
async function main() {
  ...
  for (const slug of uniquePolicySlugs) {
    rawBySlug.set(slug, await loadPolicyRaw(slug, opCounter, sourceHashes, inputResolutionRows, voidReasons));
  }

  const certificates = [];
  const rawRecomputeRows = [];
  const populationRows = populationAuditRows();
  for (const cell of CELLS) {
    ...
    rawRecomputeRows.push({
      cell_id: cell.cell_id,
      policySlug: cell.policySlug,
      selected_root: raw.selectedRoot,
      selected_base: raw.selectedBase,
  ...
  const opReport = {
    schema: "pvnp-phase2-mesa-bridge-v1-op-count-report",
    ...
    comparator_scope: "same v1 raw trace population used by the bridge certificate",
    comparator_limit: "local boundedness check only; not evidence of general verifier cheapness",
  ...
  const verdictReport = verdict(labels, decisions, audits, integrityRows, rawRecomputeRows, opReport, populationRows, inputResolutionRows, voidReasons);
  const smallRawRerunManifest = await collectSmallRawRerunManifest(inputResolutionRows);
  const completedAt = new Date().toISOString();
  const privilegeAudit = {
    schema: "pvnp-phase2-mesa-bridge-v1-privilege-audit",
  ...
  const manifest = {
    ...
    source_roots: {
      phase4_repaired_root: PHASE4_REPAIRED_ROOT,
      phase4_canonical_root: PHASE4_CANONICAL_ROOT,
  ...
  await writeJson(path.join(outDir, "source_artifact_hashes.json"), {
    schema: "pvnp-phase2-mesa-bridge-v1-source-hashes",
    artifacts: sourceHashes,
  });
  await writeJson(path.join(outDir, "verifier_access_declaration.json"), {
    schema: "pvnp-phase2-mesa-bridge-v1-access-declaration",
  ...
  await writeJson(path.join(outDir, "small_raw_rerun_manifest.json"), smallRawRerunManifest);
  await writeJson(path.join(outDir, "phase4_input_resolution.json"), {
    schema: "pvnp-phase2-mesa-bridge-v1-phase4-input-resolution",
    repaired_root: PHASE4_REPAIRED_ROOT,
    canonical_root: PHASE4_CANONICAL_ROOT,
    fallback_rule: "repaired root wins if present; repaired-present-invalid voids run; otherwise canonical root may be used",
    rows: inputResolutionRows,
    void_reasons: voidReasons,
  });
  await writeFile(path.join(outDir, "v0_to_v1_population_audit.csv"), toCsv(populationRows, [
    "ordinal", "v0_cell_id", "v1_cell_id", "v0_policySlug", "v1_policySlug",
    "v0_role", "v1_role", "population_preserved",
  ]), "utf8");
  ...
  await writeJson(path.join(outDir, "raw_recompute_audit.json"), {
    schema: "pvnp-phase2-mesa-bridge-v1-raw-recompute-audit",
  ...
  const summary = [
    "# Phase 2 Mesa Bridge v1 Falsifier Summary",
    "",
    ...
    `- small raw repair slugs resolved: ${verdictReport.counts.small_raw_repair_slugs_resolved}/${SMALL_RAW_SLUGS.size}`,
    `- population rows preserved: ${verdictReport.counts.population_rows}/${CELLS.length}`,
    "",
    "## Cost",
    "",
    ...
    "## Boundary",
    "",
    "This run detects signature-signal-control vs fixed-attractor control from raw trial logs. It does not detect literal reward-training status, does not claim mesa-general verification, and does not demonstrate body-resistance / Sundog-regime-2 behavior.",
    "",
  ].join("\n");
  await writeFile(path.join(outDir, "falsifier_summary.md"), summary, "utf8");

  const readme = [
    "# Phase 2 Mesa Bridge v1 Artifacts",
    "",
    "Generated by `scripts/pvnp-phase2-mesa-bridge-v1.mjs`.",
    "",
    "The bridge reads repaired Small raw logs, existing Medium mesa Phase 4 raw trial logs, and Phase 5 scoring labels. It does not rerun mesa training.",
    "",
    "Reproduce:",
    "",
    "```bash",
    ...
    "```",
    "",
    "See `docs/pvnp/PHASE2_MESA_BRIDGE_V1_SLATE.md` for the frozen slate.",
    "",
  ].join("\n");
  await writeFile(path.join(outDir, "README.md"), readme, "utf8");

  console.log(`phase2-mesa-bridge-v1: ${verdictReport.verdict}`);
