...
// read. This script enforces that ordering STRUCTURALLY, not just by
// comment — the calibrate() function does not accept the scored feature
// at all; admit() is a separate function that takes the precomputed
// Calibration object and the scored feature. There is no API path that
// lets the scored feature leak into the span computation.
//
// Modes:
//   check --sidecar <path>                Run the full H0 check on a
//                                          sidecar. Emits the H0 record.
//   self-test                              Run the H0-B negative side
//                                          (Phase-15 fixture) using
//                                          test sidecars that encode the
//                                          documented Phase-15 failure
//                                          modes. Verifies all 8 frames
//                                          emit admit=false with
//                                          reason_code ∈ {SPAN_TOO_SHORT,
//                                          ANCHOR_OFF_RULER}.
...
// covers only the negative side and proves the predicate's reject-on-
// failure logic.
...
// ----------------------------------------------------------------------------
// h-leak channel rules
// ----------------------------------------------------------------------------

const H_LEAK_FIELD_NAMES = new Set([
  "h", "true_h", "h_deg", "sun_altitude_deg", "altitude_deg",
  "solar_altitude", "hidden_h", "hidden_altitude",
]);
const H_LEAK_FILENAME_PATTERNS = [
  /^|[_-](h|altitude|solar_alt|sun_alt)\d+([._-]|$)/i, // e.g. _h25_, _altitude30_
  /(^|[_-])h_deg/i,
];
...
function checkHLeak(sidecar) {
  // Returns { has_leak, channels: [...] }. Inspects: filename, sidecar
  // field names, sidecar.overlay_text (operator-extracted), sidecar.
  // embedded_metadata (operator-extracted). Each channel is documented;
  // any positive ⇒ admit=false, reason_code=H_LEAK.
  const channels = [];
  ...
  // Sidecar field names (forbid any field whose name encodes h)
  ...
  // Overlay text (operator-extracted, optional)
  ...
  // Embedded metadata (operator-extracted, optional)
...
function buildThetaMap(sidecar) {
  // Returns a function px_from_sun ↦ deg_from_sun PLUS the calibrated
  // angular extent of the instrument (independent of any scored feature).
  // Three allowed kinds per P2_CUT3_H0_CALIBRATION.md §1:
  //   scale_ticks       — sidecar.scale_ticks: [{px_radius, deg}, ...]
  //   renderer_metadata — sidecar.renderer_metadata: { hash, extent_deg }
  //   fit2locus         — sidecar.fit2locus: { anchors: [{deg, px_radius}, ...] }
  ...
  if (kind === "scale_ticks") {
    ...
    return {
      ...
      pxToDeg(px_radius) {
        // piecewise-linear interpolation across the tick array
  ...
  if (kind === "renderer_metadata") {
    ...
    return {
      ...
      pxToDeg(_px_radius) {
        // External metadata file holds the actual map; the checker does
        // not reimplement it. Sidecar must declare anchor measurements
        // already in deg via .anchors[].measured_deg.
        return null;
      },
    };
  }
  if (kind === "fit2locus") {
    ...
    // Linear fit deg = a · px_radius + b (least squares).
    ...
    const extent_deg = Math.max(...ys); // largest declared anchor
    return {
      kind,
...
function calibrate(sidecar) {
  ...
  // calibrated extent — angular_extent_deg from theta_map, NEVER from
  // any feature read. This is the §2 anti-self-seal invariant.
...
function admit(calibration, scored_feature_deg) {
  ...
  // SPAN_TOO_SHORT: the calibrated angular extent doesn't even cover
  // the named atmospheric-optics anchor loci (22° / 46°). Phase-15
  // failure mode.
  for (const a of calibration.anchors) {
  ...
  // Dedupe and choose primary reason_code (first non-OK in priority order)
...
function buildPhase15TestSidecar(fixtureFrame) {
  // Each Phase-15 frame's stamped ruler covers only a short angular
  // extent (the pyramidal-zoomed crystal field). Model: ruler covers
  // 5°→18° via scale_ticks; 22° and 46° anchors are explicitly off-ruler.
  return {
    frame_id: fixtureFrame.frame_id,
    frame_path: fixtureFrame.scale_stamped_render.path,
    ...
    anchors: [
      { locus_deg: 22 }, // off-ruler (>18°), measured_deg undefined
      { locus_deg: 46 }, // off-ruler (>18°)
    ],
  };
}
...
async function selfTest() {
  ...
  for (const frame of fixture.fixture_objects) {
    ...
    const record = admit(calibration, 18.5); // any feature position in span
  ...
  console.log(
    `[h0-checker self-test] wrote ${SELF_TEST_OUT_REL}`
  );
  console.log(
