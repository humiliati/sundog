#!/usr/bin/env node
// scripts/yang-mills-phase2-v6-finite-t-polyakov.mjs
//
// Yang-Mills Phase 2 v6 - finite-temperature Polyakov pilot, ensemble build,
// powered-target audit, and relative-locality aggregate.
...
import {
  ...
  computePolyakovFiniteT,
  applySU2GaugeTransform,
  randomGaugeQuaternionsAsym,
...
const LOCKED = Object.freeze({
  ...
  cell: "SU2_3D",
  latticeSize: "12x12x4",
  outRequired: "results/yang-mills/phase2/SU2_3D/2026-05-31_su2_3d_finite_t_polyakov_v6a",
  Lx: 12,
  Ly: 12,
  Lz: 4,
  temporalMu: 2,
  ...
  pilotBetaGrid: [6.0, 6.3, 6.55, 6.8, 7.1, 7.4, 7.7, 8.0],
  pilotScanBurnIn: 800,
  pilotScanMeasurements: 96,
  pilotScanThinning: 4,
  burnIn: 2000,
  pilotSweeps: 512,
  thinning: 32,
  ...
  seed: 202605310600,
  signatureVocab: "v1",
  signatureSource: "finite_t_generated",
  heldoutVocab: "v4",
  splitRule: "transverse_site_parity",
  distanceMetric: "euclidean_zscore",
  kSlate: [3, 5, 10],
  binConvention: "per_beta_tertile_linear",
  gaugeRandSeedTag: "phase2_v6_finite_t_aggregation",
  gaugeTransformsPerConfig: 1,
});

let BETA_KEYS = Object.freeze([]);
...
const CANDIDATE_TARGETS = Object.freeze(["abs_mean_P", "mean_abs_P", "chi_P"]);
const PRIOR_TARGETS = Object.freeze(["gamma_held"]);
...
const POLYAKOV_SAMPLE_COUNT = LOCKED.Lx * LOCKED.Ly;
const LOOP_SAMPLE_COUNT = LOCKED.Lx * LOCKED.Ly * LOCKED.Lz * 3;
...
function validateLockedArgs(args) {
  ...
  check("pilot-beta-grid", LOCKED.pilotBetaGrid.map(betaKey).join(","), args["pilot-beta-grid"]);
...
function betaKey(beta) {
  return Number(beta).toFixed(2).replace(/0$/, "");
}
...
function polyakovTargetValueFromSummaries(target, summaries, halfKey = "full") {
  const summary = summaries[halfKey];
  if (target === "abs_mean_P") return { value: summary.abs_mean_P, undefined: false };
  if (target === "mean_abs_P") return { value: summary.mean_abs_P, undefined: false };
  if (target === "chi_P") return { value: summary.chi_P, undefined: false };
  throw new Error(`unknown Polyakov target ${target}`);
}

function auditTargetValue(target, record, halfKey = "full") {
  if (CANDIDATE_TARGETS.includes(target)) {
    return polyakovTargetValueFromSummaries(target, record.polyakov, halfKey);
  }
  return targetValueFromSummaries(target, record.v4Loops, halfKey);
}
...
function evaluateAuditTarget(target, records, gates) {
  ...
  for (const betaKey of BETA_KEYS) {
    ...
    for (const r of betaRecords) {
      const full = auditTargetValue(target, r, "full");
      const halfA = auditTargetValue(target, r, "halfA");
      const halfB = auditTargetValue(target, r, "halfB");
...
function compareV0V6(scoreRows, primaryTarget) {
  ...
  for (const row of scoreRows) {
    ...
    rows.push({
      ...
      v6PrimaryTarget: primaryTarget,
      v6MeanBinPurity: v6Mean,
      deltaV6MinusV0: v0Mean === undefined ? null : v6Mean - v0Mean,
    });
  }
...
function assessStage2Branch(inputs) {
  ...
  if (!inputs.v6BinEdgeTimestampOk) return ["Z void_run", "v6 bin-edge timestamp drift"];
  ...
  return ["P2-A bounded_positive", "every Phase 2 v6 point-estimate gate passes on a powered finite-T Polyakov target"];
}

function stage1Branch(admittedTarget, gammaAudit, polyakovGaugeResidualMax) {
  if (polyakovGaugeResidualMax > GAUGE_RESIDUAL_TOL) {
    return ["YM-P1-NEG-A gauge_leakage", `Polyakov target gauge residual ${polyakovGaugeResidualMax}`];
  }
  if (gammaAudit.poweredAllBetas) {
    return ["Z void_run", "gamma_held passed the v6 power self-validation gate"];
  }
...
function computePolyakovGaugeHealth(records) {
  let polyakovGaugeResidualMax = 0;
  const perTargetMaxResidual = Object.fromEntries(CANDIDATE_TARGETS.map((target) => [target, 0]));
  for (const r of records) {
    const rng = mulberry32(deriveSubstreamSeed(LOCKED.seed, LOCKED.gaugeRandSeedTag, r.betaKey, r.configIdx));
    const transformed = applySU2GaugeTransform(r.config, randomGaugeQuaternionsAsym(LOCKED.Lx, LOCKED.Ly, LOCKED.Lz, rng));
    const gaugePolyakov = computePolyakovFiniteT(transformed, LOCKED.temporalMu);
    for (const target of CANDIDATE_TARGETS) {
      const original = polyakovTargetValueFromSummaries(target, r.polyakov, "full").value;
      const gauge = polyakovTargetValueFromSummaries(target, gaugePolyakov, "full").value;
      const residual = Math.abs(gauge - original);
      perTargetMaxResidual[target] = Math.max(perTargetMaxResidual[target], residual);
      polyakovGaugeResidualMax = Math.max(polyakovGaugeResidualMax, residual);
    }
  }
  return { polyakovGaugeResidualMax, perTargetMaxResidual };
}
...
function writeStage1Summary({
  ...
  const branchInputs = {
    ...
    polyakovGaugeResidualMax: polyakovGaugeHealth.polyakovGaugeResidualMax,
    polyakovGaugeResidualByTarget: polyakovGaugeHealth.perTargetMaxResidual,
    thresholds: {
      powerIccGate: LOCKED.powerIccGate,
      powerAgreementGate: LOCKED.powerAgreementGate,
      leakageCvR2Gate: LOCKED.leakageCvr2Gate,
      polyakovGaugeResidualMax: GAUGE_RESIDUAL_TOL,
  ...
  writeJSON(path.join(aggDir, "manifest.json"), {
    ...
    phaseVersion: LOCKED.phaseVersion,
    cell: LOCKED.cell,
    latticeSize: [LOCKED.Lx, LOCKED.Ly, LOCKED.Lz],
    temporalMu: LOCKED.temporalMu,
    ...
    heldOutTargetSampleCountPerConfig: POLYAKOV_SAMPLE_COUNT,
    heldOutPriorTargetSampleCountPerConfig: LOOP_SAMPLE_COUNT,
...
function safeBetaLabel(beta) {
  return betaKey(beta).replace(".", "p");
}

function selectFrozenBetaSlate(pilotRows) {
  let peakIndex = 0;
  for (let i = 1; i < pilotRows.length; i++) {
    if (pilotRows[i].orderSusceptAbsMeanP > pilotRows[peakIndex].orderSusceptAbsMeanP) peakIndex = i;
  }
  if (peakIndex === 0 || peakIndex === pilotRows.length - 1) {
    return { peakIndex, peakBracketed: false, betaSlate: [] };
  }
  return {
    peakIndex,
    peakBracketed: true,
    betaSlate: [
      pilotRows[peakIndex - 1].beta,
      pilotRows[peakIndex].beta,
      pilotRows[peakIndex + 1].beta,
    ],
  };
}

function computeV6Polyakov(state) {
  return computePolyakovFiniteT(state, LOCKED.temporalMu);
}

function runPilotBetaScan(outDir) {
  const pilotDir = path.join(outDir, "pilot_beta_scan");
  ensureDir(pilotDir);
  const rows = [];
  const started = performance.now();
  for (const beta of LOCKED.pilotBetaGrid) {
    const key = betaKey(beta);
    const state = createSU2LatticeAsym(
      LOCKED.Lx,
      LOCKED.Ly,
      LOCKED.Lz,
      deriveSubstreamSeed(LOCKED.seed, "pilot", key, "init"),
    );
    const rng = mulberry32(deriveSubstreamSeed(LOCKED.seed, "pilot", key, "sweep"));
    const stats = makeStats();
    for (let s = 0; s < LOCKED.pilotScanBurnIn; s++) {
      combinedSweep(state, beta, LOCKED.overrelaxPerHeatbath, rng, stats);
    }
    const summaryRows = [];
    const absMean = [];
    const meanAbs = [];
    const chi = [];
    const plaquette = [];
    for (let m = 0; m < LOCKED.pilotScanMeasurements; m++) {
      for (let s = 0; s < LOCKED.pilotScanThinning; s++) {
        combinedSweep(state, beta, LOCKED.overrelaxPerHeatbath, rng, stats);
      }
      const p = computeV6Polyakov(state).full;
      const by = meanPlaquetteByOrientation(state);
      const meanPlaq = (by.xy + by.xz + by.yz) / 3;
      absMean.push(p.abs_mean_P);
      meanAbs.push(p.mean_abs_P);
      chi.push(p.chi_P);
      plaquette.push(meanPlaq);
      summaryRows.push([m, p.abs_mean_P, p.mean_abs_P, p.chi_P, p.mean_P, meanPlaq]);
    }
    const absMeanStats = valueStats(absMean);
    const meanAbsStats = valueStats(meanAbs);
    writeCSV(
      path.join(pilotDir, `beta${safeBetaLabel(beta)}_polyakov_series.csv`),
      ["sample_idx", "abs_mean_P", "mean_abs_P", "chi_P", "mean_P", "mean_plaquette"],
      summaryRows,
    );
    rows.push({
      beta,
      betaKey: key,
      meanAbsMeanP: mean(absMean),
      meanMeanAbsP: mean(meanAbs),
      meanChiP: mean(chi),
      orderSusceptAbsMeanP: POLYAKOV_SAMPLE_COUNT * absMeanStats.variance,
      orderSusceptMeanAbsP: POLYAKOV_SAMPLE_COUNT * meanAbsStats.variance,
      varianceAbsMeanP: absMeanStats.variance,
      varianceMeanAbsP: meanAbsStats.variance,
      meanPlaquette: mean(plaquette),
      heatbathFallbackFraction: fallbackFraction(stats),
      heatbathLinkUpdates: stats.heatbathLinkUpdates,
    });
  }
  const selected = selectFrozenBetaSlate(rows);
  const payload = {
    frozenAt: new Date().toISOString(),
    literatureAnchor: {
      betaC: 6.53661,
      betaCUncertainty: 0.00013,
      source: "Edwards-von Smekal 2009 arXiv:0908.4030",
      convention: "beta=4/g^2 a",
    },
    pilotGrid: LOCKED.pilotBetaGrid,
    pilotScan: {
      burnIn: LOCKED.pilotScanBurnIn,
      measurements: LOCKED.pilotScanMeasurements,
      thinning: LOCKED.pilotScanThinning,
      selectionMetric: "order_suscept_abs_mean_P",
      selectionMetricDefinition: "V_perp * Var(abs_mean_P) across pilot measurements; V_perp=12*12",
      recordOnlyPriorMetric: "mean_chi_P",
    },
    rows,
    peakIndex: selected.peakIndex,
    peakBracketed: selected.peakBracketed,
    frozenBetaSlate: selected.betaSlate,
    wallClockSeconds: (performance.now() - started) / 1000,
  };
  writeCSV(
    path.join(pilotDir, "pilot_beta_scan.csv"),
    [
      "beta",
      "mean_abs_mean_P",
      "mean_mean_abs_P",
      ...
      "order_suscept_mean_abs_P",
      "variance_abs_mean_P",
      "variance_mean_abs_P",
      "mean_plaquette",
      "heatbath_fallback_fraction",
      "heatbath_link_updates",
    ],
    rows.map((r) => [
      r.betaKey,
      r.meanAbsMeanP,
      r.meanMeanAbsP,
      r.meanChiP,
      r.orderSusceptAbsMeanP,
      r.orderSusceptMeanAbsP,
      r.varianceAbsMeanP,
      r.varianceMeanAbsP,
      r.meanPlaquette,
      r.heatbathFallbackFraction,
      r.heatbathLinkUpdates,
    ]),
  );
  writeJSON(path.join(pilotDir, "frozen_beta_slate.json"), payload);
  return payload;
}

function finiteTEnsembleDir(outDir, beta) {
  return path.join(outDir, "ensembles", `beta${safeBetaLabel(beta)}`);
}

function buildEnsembleManifest({ beta, betaIndex, seed, tauResult, wallClockSeconds, codeInfo, stats, maxUnitResidual, orientationMeans, orientationVariances, orientationSpread, artifactHashes }) {
  return {
    phase: "phase2",
    phaseVersion: LOCKED.phaseVersion,
    cell: LOCKED.cell,
    ...
    signatureVocabularyVersion: LOCKED.signatureVocab,
    heldOutTargetVocabularyVersion: LOCKED.heldoutVocab,
    temporalPolyakovSampleCountPerConfig: POLYAKOV_SAMPLE_COUNT,
    gammaHeldEpsilonFloor: GAMMA_EPSILON,
    ...
    perOrientationRelativeSpreadRecordOnlyFiniteT: orientationSpread,
    codeCommit: codeInfo.codeCommit,
    gitDirty: codeInfo.gitDirty,
    commandLine: reconstructCommandLine(),
    wallClockSeconds,
...
function runFiniteTEnsemble({ outDir, beta, betaIndex, startIndex }) {
  const seed = LOCKED.seed + betaIndex;
  const ensembleDir = finiteTEnsembleDir(outDir, beta);
  ensureCleanDir(
    ensembleDir,
    `${LOCKED.outRequired}/ensembles/beta${safeBetaLabel(beta)}`,
  );
  for (const sub of ["autocorr_pilot", "configs", "signatures", "heldout"]) {
    ensureDir(path.join(ensembleDir, sub));
  }
  const start = performance.now();
  const betaLabel = betaKey(beta);
  const state = createSU2LatticeAsym(LOCKED.Lx, LOCKED.Ly, LOCKED.Lz, deriveSubstreamSeed(seed, "init"));
  const rng = mulberry32(deriveSubstreamSeed(seed, "sweep"));
  const stats = makeStats();

  console.log(`[YM-P2-V6] ensemble beta=${betaLabel} seed=${seed}: burn-in ${LOCKED.burnIn}`);
  for (let s = 0; s < LOCKED.burnIn; s++) {
    combinedSweep(state, beta, LOCKED.overrelaxPerHeatbath, rng, stats);
  }

  const pilotSeries = new Float64Array(LOCKED.pilotSweeps);
  const pilotXY = new Float64Array(LOCKED.pilotSweeps);
  const pilotXZ = new Float64Array(LOCKED.pilotSweeps);
  const pilotYZ = new Float64Array(LOCKED.pilotSweeps);
  ...
  const xyStats = valueStats(Array.from(pilotXY));
  const xzStats = valueStats(Array.from(pilotXZ));
  const yzStats = valueStats(Array.from(pilotYZ));
  ...
  writeCSV(path.join(ensembleDir, "autocorr_pilot", "plaquette_series.csv"), ["sweep_index", "mean_plaquette"], Array.from(pilotSeries, (v, i) => [i, v]));
  writeCSV(path.join(ensembleDir, "autocorr_pilot", "plaquette_by_orientation.csv"), ["sweep_index", "xy", "xz", "yz"], Array.from(pilotSeries, (_, i) => [i, pilotXY[i], pilotXZ[i], pilotYZ[i]]));
  ...
  for (let m = 0; m < LOCKED.measurements; m++) {
    ...
    const gamma = gammaHeld(held, GAMMA_EPSILON);
    const polyakov = computeV6Polyakov(cfg);
    const signatureVector = signatureVectorFromObject(sig);
    signatureRows.push([m, ...Array.from(signatureVector)]);
    ...
    polyakovRows.push([m, polyakov.full.abs_mean_P, polyakov.full.mean_abs_P, polyakov.full.chi_P, polyakov.full.mean_P, polyakov.halfA.n, polyakov.halfB.n]);
    records.push({
      globalIndex: startIndex + m,
      id: `beta${betaLabel}_config${m}`,
      betaKey: betaLabel,
      beta,
      configIdx: m,
      signature: signatureVector,
      config: cfg,
      sourceDir: ensembleDir,
    });
  }

  writeText(
    path.join(ensembleDir, "configs", "su2_links.jsonl"),
    configs.map((cfg, configIdx) => JSON.stringify({
      configIdx,
      shape: [3, LOCKED.Lx, LOCKED.Ly, LOCKED.Lz, 4],
      quaternionLinks: Array.from(cfg.links),
    })).join("\n") + "\n",
  );
  writeCSV(path.join(ensembleDir, "signatures", "signature_vectors.csv"), ["config_idx", ...SIGNATURE_KEYS], signatureRows);
  writeCSV(path.join(ensembleDir, "heldout", "heldout_loop_values.csv"), ["config_idx", "W14_mean", "W14_var", "W23_mean", "W23_var", "W33_mean", "W33_var"], heldoutRows);
  writeCSV(path.join(ensembleDir, "heldout", "heldout_summary.csv"), ["config_idx", "W14_mean", "W23_mean", "W33_mean", "gamma_held", "clamped"], heldoutSummaryRows);
  writeCSV(path.join(ensembleDir, "heldout", "finite_t_polyakov_summary.csv"), ["config_idx", "abs_mean_P", "mean_abs_P", "chi_P", "mean_P", "half_A_sample_count", "half_B_sample_count"], polyakovRows);
  ...
  let reason = "all finite-T ensemble health thresholds pass";
  if (!unitOk) {
  ...
  const summary = {
    ...
    orientation: { meanPlaquette: orientationMeans, variancePlaquette: orientationVariances, relativeSpreadRecordOnlyFiniteT: orientSpread },
    measurements: { retained: configs.length },
    heldout: { gammaHeldEpsilonFloor: GAMMA_EPSILON, clampCount: heldoutSummaryRows.filter((r) => r[5] === 1).length },
    polyakov: {
      temporalMu: LOCKED.temporalMu,
      sampleCountPerConfig: POLYAKOV_SAMPLE_COUNT,
      meanAbsMeanP: mean(polyakovRows.map((r) => r[1])),
      meanMeanAbsP: mean(polyakovRows.map((r) => r[2])),
      meanChiP: mean(polyakovRows.map((r) => r[3])),
    },
  ...
  writeJSON(path.join(ensembleDir, "summary.json"), summary);
  const artifactHashes = collectHashes(ensembleDir, new Set(["manifest.json", "hashes.json"]));
  const manifest = buildEnsembleManifest({ beta, betaIndex, seed, tauResult, wallClockSeconds, codeInfo: getGitInfo(), stats, maxUnitResidual, orientationMeans, orientationVariances, orientationSpread: orientSpread, artifactHashes });
  writeJSON(path.join(ensembleDir, "manifest.json"), manifest);
  finalizeHashes(ensembleDir);
  ...
  const source = {
    ...
    betaKey: betaLabel,
    betaIndex,
    resultDir: ensembleDir.replace(/\\/g, "/"),
    manifestCodeCommit: manifest.codeCommit,
    manifestGitDirty: manifest.gitDirty,
    summaryBranch: summary.branch,
    configsSha256: sha256OfFile(path.join(ensembleDir, "configs", "su2_links.jsonl")),
    signatureVectorsSha256: sha256OfFile(path.join(ensembleDir, "signatures", "signature_vectors.csv")),
    polyakovSummarySha256: sha256OfFile(path.join(ensembleDir, "heldout", "finite_t_polyakov_summary.csv")),
  };
  console.log(`[YM-P2-V6] ensemble beta=${betaLabel} verdict: ${branch}; tau=${tauResult.tauInt.toFixed(3)}; wall=${wallClockSeconds.toFixed(2)}s`);
  return { records, source };
}
...
async function main() {
  ...
  if (failures.length > 0) {
    console.error("[YM-P2-V6] manifest validation FAILED:");
  ...
  ensureCleanDir(outDir, LOCKED.outRequired);
  ensureDir(aggDir);

  console.log("[YM-P2-V6] Pilot: scanning finite-T Polyakov susceptibility over locked beta grid");
  const betaPilot = runPilotBetaScan(outDir);
  if (!betaPilot.peakBracketed) {
    const wallClockSeconds = (performance.now() - start) / 1000;
    writeJSON(path.join(aggDir, "summary.json"), {
      branch: "Z beta_peak_unbracketed",
      reason: "pilot Polyakov susceptibility peak landed on the boundary of the locked grid",
      phaseVersion: LOCKED.phaseVersion,
      rankLocalityScored: false,
      betaPilot,
      wallClockSeconds,
    });
    writeJSON(path.join(aggDir, "manifest.json"), {
      ...
      latticeSize: [LOCKED.Lx, LOCKED.Ly, LOCKED.Lz],
      betaPilot,
      codeCommit: getGitInfo().codeCommit,
      gitDirty: getGitInfo().gitDirty,
      commandLine: reconstructCommandLine(),
      wallClockSeconds,
    });
    finalizeHashes(outDir);
    console.log("[YM-P2-V6] verdict: Z beta_peak_unbracketed");
    return;
  }
  BETA_KEYS = Object.freeze(betaPilot.frozenBetaSlate.map(betaKey));
  writeJSON(path.join(aggDir, "frozen_beta_slate.json"), betaPilot);
  const pilotSweepCount = LOCKED.pilotBetaGrid.length * (LOCKED.pilotScanBurnIn + LOCKED.pilotScanMeasurements * LOCKED.pilotScanThinning);
  const ensembleSweepCount = 3 * (LOCKED.burnIn + LOCKED.pilotSweeps + LOCKED.measurements * LOCKED.thinning);
  const elapsedAfterPilot = (performance.now() - start) / 1000;
  const predictedTotal = elapsedAfterPilot + (elapsedAfterPilot / pilotSweepCount) * ensembleSweepCount + 30;
  if (predictedTotal > THRESH.wallClockSeconds) {
    ...
    finalizeHashes(outDir);
    console.log("[YM-P2-V6] verdict: Z void_run");
    console.log(`[YM-P2-V6] predicted total ${predictedTotal.toFixed(2)}s exceeds cap`);
    return;
  }

  console.log(`[YM-P2-V6] frozen beta slate: ${BETA_KEYS.join(", ")}`);
  let records = [];
  const v6EnsembleSources = [];
  for (let i = 0; i < betaPilot.frozenBetaSlate.length; i++) {
    const read = runFiniteTEnsemble({ outDir, beta: betaPilot.frozenBetaSlate[i], betaIndex: i, startIndex: records.length });
    records = records.concat(read.records);
    v6EnsembleSources.push(read.source);
  }
  if (records.length !== 96) throw new Error(`expected 96 records, got ${records.length}`);
  writeJSON(path.join(aggDir, "v6_ensemble_sources.json"), v6EnsembleSources);

  const badEnsemble = v6EnsembleSources.find((s) => s.summaryBranch !== "P1-A ensemble_health_pass");
  if (badEnsemble) {
    const wallClockSeconds = (performance.now() - start) / 1000;
    writeJSON(path.join(aggDir, "summary.json"), {
      branch: badEnsemble.summaryBranch,
      reason: `finite-T ensemble health failed for beta ${badEnsemble.betaKey}`,
      phaseVersion: LOCKED.phaseVersion,
      rankLocalityScored: false,
      betaPilot,
      badEnsemble,
    ...
    finalizeHashes(outDir);
    console.log(`[YM-P2-V6] verdict: ${badEnsemble.summaryBranch}`);
    return;
  }

  console.log("[YM-P2-V6] Stage 1: computing finite-T temporal Polyakov summaries for powered-target audit");
  const targetRows = [];
  for (const r of records) {
    r.v4Loops = computeV4LoopSummaries(r.config);
    r.polyakov = computeV6Polyakov(r.config);
    const values = {};
    for (const target of ALL_AUDIT_TARGETS) values[target] = auditTargetValue(target, r, "full").value;
    targetRows.push([
      r.betaKey,
      r.configIdx,
      values.abs_mean_P,
  ...
  writeCSV(
    path.join(aggDir, "v6_finite_t_polyakov_target_resummaries.csv"),
    [
      "beta",
      "config_idx",
      "abs_mean_P",
      ...
      "gamma_held",
      "polyakov_sample_count",
      "polyakov_half_A_sample_count",
      "polyakov_half_B_sample_count",
      "gamma_held_loop_sample_count",
    ],
    targetRows,
  );
  ...
  writeJSON(path.join(aggDir, "target_power_audit.json"), {
    ...
    splitRule: LOCKED.splitRule,
    sampleCountPerConfig: POLYAKOV_SAMPLE_COUNT,
    priorTargetSampleCountPerConfig: LOOP_SAMPLE_COUNT,
    gates,
    candidatePool: CANDIDATE_TARGETS,
    priorTargetsReportedNotAdmissible: PRIOR_TARGETS,
    audits,
  ...
  const polyakovGaugeHealth = computePolyakovGaugeHealth(records);
  writeJSON(path.join(aggDir, "polyakov_gauge_health.json"), {
    seed: LOCKED.seed,
    gaugeRandSeedTag: LOCKED.gaugeRandSeedTag,
    gaugeTransformsPerConfig: LOCKED.gaugeTransformsPerConfig,
    tolerance: GAUGE_RESIDUAL_TOL,
    ...polyakovGaugeHealth,
  });

  const gammaAudit = audits.find((a) => a.target === "gamma_held");
  const [stage1OnlyBranch, stage1OnlyReason] = stage1Branch(admittedTarget, gammaAudit, polyakovGaugeHealth.polyakovGaugeResidualMax);
  if (stage1OnlyBranch) {
    const wallClockSeconds = (performance.now() - start) / 1000;
    writeStage1Summary({
      aggDir,
    ...
    finalizeHashes(outDir);
    console.log(`[YM-P2-V6] verdict: ${stage1OnlyBranch}`);
    console.log(`[YM-P2-V6] ${stage1OnlyReason}`);
    console.log(`[YM-P2-V6] wall clock ${wallClockSeconds.toFixed(2)} s`);
    return;
  }

  const primaryTarget = admittedTarget.primary.target;
  for (const r of records) {
    const target = polyakovTargetValueFromSummaries(primaryTarget, r.polyakov, "full");
  ...
  const perBetaEdgesPath = path.join(aggDir, "per_beta_v6_bin_edges.json");
  const globalEdgesPath = path.join(aggDir, "global_v6_bin_edges.json");
  ...
  console.log(`[YM-P2-V6] Stage 2: scoring admitted powered finite-T Polyakov target ${primaryTarget}`);
  ...
  for (const r of records) {
    ...
    const gaugePolyakov = computeV6Polyakov(transformed);
    const gaugeTarget = polyakovTargetValueFromSummaries(primaryTarget, gaugePolyakov, "full");
  ...
  const controls = {
    CTRL_META: {
      within: buildNearestGraph(records, normalizeFeatures(records, (r) => new Float64Array([r.beta, LOCKED.Lx, LOCKED.Ly, LOCKED.Lz])).normalized, withinCandidate, maxK),
      across: buildNearestGraph(records, normalizeFeatures(records, (r) => new Float64Array([r.beta, LOCKED.Lx, LOCKED.Ly, LOCKED.Lz])).normalized, acrossCandidate, maxK),
    },
    CTRL_RAW: {
  ...
  writeJSON(path.join(aggDir, "v0_v6_comparison.json"), compareV0V6(scoreRows, primaryTarget));
  ...
  const branchInputs = {
    ...
    v6BinEdgeTimestampOk: edgeMtimeMs < firstScoringArtifactMtimeMs,
    binDegenerateBetas,
  ...
  const summary = {
    ...
    integrity: {
      ...
      v6BinEdgeTimestampOk: branchInputs.v6BinEdgeTimestampOk,
      gaugeSignatureResidualMax,
      gaugeTargetResidualMax,
      polyakovGaugeResidualMax: polyakovGaugeHealth.polyakovGaugeResidualMax,
      polyakovGaugeResidualByTarget: polyakovGaugeHealth.perTargetMaxResidual,
    ...
    target: {
      primaryTarget,
      perBetaBinEdgesPath: "aggregation/per_beta_v6_bin_edges.json",
      globalBinEdgesPath: "aggregation/global_v6_bin_edges.json",
      binDegenerateBetas,
    },
    betaPilot,
    wallClockSeconds,
  ...
  console.log(`[YM-P2-V6] verdict: ${branch}`);
  console.log(`[YM-P2-V6] ${reason}`);
  console.log(`[YM-P2-V6] admitted target ${primaryTarget}; mean ICC ${admittedTarget.primary.meanIcc.toFixed(4)}; mean leakage ${admittedTarget.primary.meanLeakageCvR2.toFixed(4)}`);
  console.log(`[YM-P2-V6] within primary@5 ${branchInputs.withinPrimary5.toFixed(4)}; raw ${branchInputs.withinCtrlRaw5.toFixed(4)}; rand ${branchInputs.withinCtrlRand5.toFixed(4)}`);
  console.log(`[YM-P2-V6] gauge residuals signature=${gaugeSignatureResidualMax.toExponential(3)} target=${gaugeTargetResidualMax.toExponential(3)}`);
  console.log(`[YM-P2-V6] wall clock ${wallClockSeconds.toFixed(2)} s`);
}

main().catch((err) => {
  console.error("[YM-P2-V6] runner threw:");
