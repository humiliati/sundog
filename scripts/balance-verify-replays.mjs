...
import { parseArgs as parseHarnessArgs, runSuite } from "./balance-harness.mjs";
...
function parseArgs(argv) {
  const args = {
    source: "results/balance/phase7-smoke/replay-index.json",
    out: "results/balance/phase7-smoke/replay-verification.csv",
    manifest: "results/balance/phase7-smoke/replay-verification.json",
...
function verifyRow(row, args) {
  const harnessArgs = parseHarnessArgs([
    ...
    args.phase,
    "--out",
    "results/balance/phase7-replay-verify",
    "--replay-url",
    row.browserReplayUrl,
  ]);
  const { results } = runSuite(harnessArgs);
  const replay = results[0];
  const timeDelta = Math.abs(replay.simulatedTime - row.simulatedTime);
  const pass = replay.outcome === row.outcome && timeDelta <= args.tolerance;
  ...
  return {
    ...
    expectedSimulatedTime: row.simulatedTime,
    replaySimulatedTime: replay.simulatedTime,
    timeDelta,
...
async function main() {
  ...
  const manifest = {
    schema: "sundog.balance.phase7-replay-verification.v1",
  ...
  console.log(`Balance replay verification: ${manifest.passed}/${manifest.checked} passed`);
  console.log(`Wrote ${path.relative(repoRoot, outPath)}`);
  if (failures.length > 0) {
    console.error(`Replay verification failed for ${failures.length} trial(s)`);
    for (const row of failures.slice(0, 8)) {
      console.error(`${row.trialId}: expected ${row.expectedOutcome}@${row.expectedSimulatedTime}, got ${row.replayOutcome}@${row.replaySimulatedTime}`);
    }
    process.exitCode = 1;
  }
}
