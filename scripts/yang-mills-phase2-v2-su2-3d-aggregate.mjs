#!/usr/bin/env node
// scripts/yang-mills-phase2-v2-su2-3d-aggregate.mjs
//
// Yang-Mills Phase 2 v2 - SU(2) 3D relative-locality aggregation runner
// with vocab v5 connected 2-point correlator signatures.
...
import {
  computeCorrelatorSignatureV5,
  correlatorSignatureKeys,
  displacementClasses,
} from "./lib/yang-mills-su2-3d-correlator.mjs";
...
const LOCKED = Object.freeze({
  ...
  signatureVocab: "v5",
  signatureLoops: "W11,W12,W13,W22",
  displacementClasses: "1-0-0,1-1-0,1-1-1,2-0-0,2-1-0",
  distanceMetric: "euclidean_zscore",
  kSlate: "3,5,10",
  primaryK: 5,
  bootstrapResamples: 1000,
  binConvention: "per_beta_tertile_linear",
  gaugeRandSeedTag: "phase2_v2_aggregation",
  ...
  outRequired: "results/yang-mills/phase2/SU2_3D/2026-05-29_su2_3d_relative_locality_v2",
  inputs: {
...
const MASTER_SEED = 202605290499;
const EDGE_MATCH_TOL = 1e-12;
const GAUGE_SIGNATURE_TOL = 1e-12;
const AGGREGATION_WALL_CLOCK_MAX = 600;
const SIGNATURE_KEYS = correlatorSignatureKeys();
...
function validateLockedArgs(args) {
  ...
  check("signature-loops", LOCKED.signatureLoops, args["signature-loops"]);
  check("displacement-classes", LOCKED.displacementClasses, args["displacement-classes"]);
...
function readEnsemble(betaKey, dir, startIndex) {
  ...
  if (held.length !== 32 || configs.length !== 32) throw new Error(`v0 ensemble ${betaKey} expected 32 held/configs`);
  const records = held.map((h, localIdx) => {
    const configIdx = Number(h.config_idx);
    const parsedConfig = JSON.parse(configs[localIdx]);
    if (parsedConfig.configIdx !== configIdx) throw new Error(`config index drift for beta ${betaKey}: held ${configIdx}, jsonl ${parsedConfig.configIdx}`);
    return {
...
function compareEdgesToV0(perBetaEdges, globalEdges, v0PerBetaPath) {
  ...
  const globalMaxAbsDiff = Math.max(Math.abs(globalEdges.lowEdge - v0Global.lowEdge), Math.abs(globalEdges.highEdge - v0Global.highEdge));
  return {
...
function compareV0V1V2(scoreRows) {
  const v0ScorePath = path.join(path.dirname(LOCKED.v0BinEdges), "rank_locality_scores.csv");
  const v1ScorePath = "results/yang-mills/phase2/SU2_3D/2026-05-29_su2_3d_relative_locality_v1/aggregation/rank_locality_scores.csv";
  const v0 = readScoreMap(v0ScorePath);
  const v1 = readScoreMap(v1ScorePath);
  const rows = [];
  for (const row of scoreRows) {
    const [lane, id, k, v2Mean] = row;
    const key = `${lane}:${id}:${k}`;
    const v0Mean = v0.get(key);
    const v1Mean = v1.get(key);
    rows.push({
      lane,
      controlOrPrimary: id,
      k,
      v0MeanBinPurity: v0Mean ?? null,
      v1MeanBinPurity: v1Mean ?? null,
      v2MeanBinPurity: v2Mean,
      deltaV2MinusV0: v0Mean === undefined ? null : v2Mean - v0Mean,
      deltaV2MinusV1: v1Mean === undefined ? null : v2Mean - v1Mean,
    });
  }
  return {
    v0ScorePath: v0ScorePath.replace(/\\/g, "/"),
    v1ScorePath: v1ScorePath.replace(/\\/g, "/"),
    rows,
  };
}
...
function assessBranch(inputs) {
  ...
  if (!inputs.v0BinEdgeTimestampOk) return ["Z void_run", "v0 bin-edge timestamp is not earlier than v2 start"];
  ...
  if (inputs.gaugeRandPurityDiff > 1e-12 || inputs.gaugeSignatureResidualMax > GAUGE_SIGNATURE_TOL) {
  ...
  return ["P2-A bounded_positive", "every Phase 2 v2 point-estimate gate passes"];
}
...
async function main() {
  ...
  if (failures.length > 0) {
    console.error("[YM-P2-V2-AGG] manifest validation FAILED:");
  ...
  ensureCleanDir(aggDir, "results/yang-mills/phase2/SU2_3D/2026-05-29_su2_3d_relative_locality_v2/aggregation");
  ...
  const v0BinEdgeTimestampOk = fs.statSync(v0BinEdgesPath).mtimeMs <= startWallMs && fs.statSync(v0GlobalBinEdgesPath).mtimeMs <= startWallMs;
  ...
  writeJSON(path.join(aggDir, "displacement_classes.json"), displacementClasses());

  console.log("[YM-P2-V2-AGG] computing v5 correlator signatures for 96 configs");
  const signatureRows = [];
  for (const r of records) {
    const sig = computeCorrelatorSignatureV5(r.config);
    r.signature = sig.vector;
    signatureRows.push([r.betaKey, r.configIdx, ...Array.from(r.signature)]);
  }
  writeCSV(path.join(aggDir, "correlator_signature_vectors.csv"), ["beta", "config_idx", ...SIGNATURE_KEYS], signatureRows);
  ...
  console.log("[YM-P2-V2-AGG] gauge-randomized correlator control");
  ...
  for (const r of records) {
    ...
    const sig = computeCorrelatorSignatureV5(transformed).vector;
    gaugeSignatures[r.globalIndex] = sig;
    gaugeSignatureResidualMax = Math.max(gaugeSignatureResidualMax, maxAbsVectorDiff(r.signature, sig));
  }
  ...
  writeJSON(path.join(aggDir, "v0_v1_v2_comparison.json"), compareV0V1V2(scoreRows));
  ...
  branchInputs.thresholds = {
    ...
    gaugeSignatureResidualMax: GAUGE_SIGNATURE_TOL,
  ...
  const manifest = {
    ...
    displacementClassRepresentatives: [[1, 0, 0], [1, 1, 0], [1, 1, 1], [2, 0, 0], [2, 1, 0]],
    signatureDimension: SIGNATURE_KEYS.length,
  ...
  console.log(`[YM-P2-V2-AGG] verdict: ${branch}`);
  console.log(`[YM-P2-V2-AGG] ${reason}`);
  console.log(`[YM-P2-V2-AGG] within primary@5 ${branchInputs.withinPrimary5.toFixed(4)}; raw ${branchInputs.withinCtrlRaw5.toFixed(4)}; rand ${branchInputs.withinCtrlRand5.toFixed(4)}`);
  console.log(`[YM-P2-V2-AGG] gauge residual ${gaugeSignatureResidualMax.toExponential(3)}`);
  console.log(`[YM-P2-V2-AGG] wall clock ${wallClockSeconds.toFixed(2)} s`);
}

main().catch((err) => {
  console.error("[YM-P2-V2-AGG] runner threw:");
