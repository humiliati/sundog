...
async function main() {
  ...
  let inversionExists = false;
  try { await access(inversionPath, fsConstants.F_OK); inversionExists = true; } catch {}
  let inversionWallMs = 0;
  if (!args.skipInversion && !inversionExists) {
  ...
  for (const sigma of sigs) {
    ...
    rows.push(csvRow([
      "A_spoof_small", envId, policyId, publicEnv.split,
  ...
  const anyInvSuccess = inversionResults.evaluations
    .some((ev) => ev.success_count > 0);
  const capacityThreshold = (anyInvSuccess || spoofSuccess > 0)
    ? "<=small" : "not_estimated";
  ...
  console.log(`spoof: attempts=${spoofAttempts}; successes=${spoofSuccess}`);
