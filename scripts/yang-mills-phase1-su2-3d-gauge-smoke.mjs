#!/usr/bin/env node
// scripts/yang-mills-phase1-su2-3d-gauge-smoke.mjs
//
// Yang-Mills Phase 1 - SU(2) 3D gauge-invariance smoke runner.
// This runner is scoped to the SU2_3D primary instrumentation cell only.
...
async function main() {
  ...
  if (validationFailures.length > 0) {
    console.error("[YM-P1-SU2-3D] manifest validation FAILED:");
  ...
  console.log(`[YM-P1-SU2-3D] start: cell=${LOCKED.cell} L=${L} beta=${LOCKED.beta} seed=${LOCKED.seed}`);
  console.log(`[YM-P1-SU2-3D] burn-in: ${LOCKED.burnIn} combined sweeps`);
  for (let s = 0; s < LOCKED.burnIn; s++) {
    combinedSweep(state, LOCKED.beta, LOCKED.overrelaxPerHeatbath, rng, stats);
  }
  console.log(
    `[YM-P1-SU2-3D] burn-in done; fallback fraction ${fallbackFraction(stats).toExponential(3)}`,
  );

  console.log(`[YM-P1-SU2-3D] pilot: ${LOCKED.pilotSweeps} combined sweeps`);
  ...
  for (let s = 0; s < LOCKED.pilotSweeps; s++) {
    ...
    pilotXY[s] = byOrientation.xy;
    pilotXZ[s] = byOrientation.xz;
    pilotYZ[s] = byOrientation.yz;
    pilotSeries[s] = (byOrientation.xy + byOrientation.xz + byOrientation.yz) / 3;
  }
  ...
  console.log(
    `[YM-P1-SU2-3D] pilot done; mean plaq ${tauResult.mean.toFixed(6)}, tau_int ${tauResult.tauInt.toFixed(3)}, orientation spread ${orientSpread.toExponential(3)}`,
  );
  ...
  writeCSV(
    path.join(outDir, "autocorr_pilot", "plaquette_by_orientation.csv"),
    ["sweep_index", "xy", "xz", "yz"],
    Array.from(pilotSeries, (_, i) => [i, pilotXY[i], pilotXZ[i], pilotYZ[i]]),
  );
  ...
  if (predictedTotal > THRESH.wallClockSeconds) {
    ...
    writeJSON(path.join(outDir, "summary.json"), {
      ...
      orientation: {
        meanPlaquette: orientationMeans,
        variancePlaquette: orientationVariances,
        relativeSpread: orientSpread,
      },
    });
    ...
    writeJSON(
      ...
      buildManifest({
        ...
        orientationMeans,
        orientationVariances,
        orientationSpread: orientSpread,
        artifactHashes,
      }),
    );
  ...
  console.log(
    `[YM-P1-SU2-3D] measurements: ${LOCKED.measurements} configs at thinning ${LOCKED.thinning}`,
  );
  const configs = [];
  const signatureRows = [];
  const heldoutRows = [];
  ...
  writeText(
    ...
    configs
      .map((cfg, configIdx) =>
        JSON.stringify({ configIdx, shape: [3, L, L, L, 4], quaternionLinks: Array.from(cfg.links) }),
      )
      .join("\n") + "\n",
  );
  writeCSV(
  ...
  console.log(
    `[YM-P1-SU2-3D] gauge randomization: identity + ${LOCKED.gaugeTransforms} random transforms per config`,
  );
  const sigResRows = [];
  const rawResRows = [];
  ...
  if (!unitOk) {
    ...
    reason =
      "all burn-in / autocorrelation / signature-invariance / heatbath-fallback / raw-non-invariance / unitarity / orientation-isotropy thresholds pass";
  }
  ...
  const summary = {
    ...
    orientation: {
      ...
      variancePlaquette: orientationVariances,
      relativeSpread: orientSpread,
      thresholdRelativeSpread: THRESH.orientationRelativeSpreadMax,
    },
    ...
    gaugeRandomization: {
      ...
      thresholds: {
        ...
        linkUnitarityMaxFrobenius: THRESH.unitarityFrobeniusResidualMax,
      },
    },
    wallClockSeconds,
  };
  ...
  console.log(`[YM-P1-SU2-3D] verdict: ${branch}`);
  console.log(`[YM-P1-SU2-3D] ${reason}`);
  console.log(`[YM-P1-SU2-3D] wall clock ${wallClockSeconds.toFixed(2)} s`);
}

main().catch((err) => {
  console.error("[YM-P1-SU2-3D] runner threw:");
