...
- **slaving_index** = held-out R^2 of R predicted from Phi_K (1 = fully signature-determined, 0 = unpredictable): `0.9989`
- `R^2(R | Phi_K)`: `0.9989`
- `R^2(g)` positive control (g is an exact f(Phi_K); want > 0.90): `0.9994`
- `R^2(permuted R)` negative control (want < 0.10): `-0.0003587`
- train / test (block split, no temporal leakage): `35000` / `15000`

## Energy-conservation finding (v1 record)

- `rms(R)/rms(dE_low/dt)`: `28.7`  `rms(R)/rms(g)`: `0.999`  `rms(R)/rms(T_low)`: `1` (=1 => T_LLL=0)
- `corr(g,R)`: `-0.8924` (negative => quasi-equilibrium cancellation)
- `D_low` mean/max: `-0.113` / `-0.1092` (max<=0)  `F_low` mean: `0.2077` (>0)
- `damp_fraction`: `0.2967`  samples: `50000`

## Reading

**Coupling signature-slaved** (slaving_index `0.999`): the net high-mode energy-transfer `R` into the band is pinned by `Phi_K` even though the high modes themselves roam (twin states). This is the mechanism for control-sufficiency: the *relevant functional* is slaved to the signature, not the state. Explanatory; C1 unchanged.
