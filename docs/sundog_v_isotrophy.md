...
| 2026-05-22 | **v0.3h BRIDGE AUDIT COMPLETE.** `npm run isotrophy:kfacet:bridge-audit` now respects each row's selected adaptive floor, so the five repaired rows are no longer mis-flagged as bridge suspects. The manifest reads `20 no_bridge_present / 1 defective_E_block_confirmed`. `O_617` is sharply excluded from evidence: bridge SV `7.8359e-4`, neutral overlap `1.81e-4`, Jordan chain drop `90.04`, and the k=8 bridge-admitted representation fails structurally (`T(2)+S(6)+E(1)`, `P_E` marginal SV `0.01475`, `sigma3^3-I = 3.96e-2`). This closes the bridge sub-investigation as a defective D3 boundary block, not a missed neutral, Jordan block, tighter-rtol candidate, or valid standard sector. **Deep-dive upstream attribution (companion note `internal/anniversary/kfacet_v03h_o617_deep_dive.md`):** the defective-D3 is causally downstream of weak `sigma_3` catalog admission. `O_617`'s `sigma_group_residual_inf = 1.62e-1` ranks 19 of 21 against a catalog median near `2e-8`; `F_beta` closure is catalog-normal (rank 4 of 21). The bridge vector is exactly tangent to the (E, |L|) level set (cosines `~1e-10`), so the defect lives in the representation, not the orbit. Headline: **v0.3h resolves the eligible strict catalog rows as structural zeros; O_617 is explained by weak sigma_3 catalog inclusion, not by a Gamma_i failure.** Honest follow-up is a catalog-reconstruction question (tighter sigma_3 admission), not a v0.3h question. |
