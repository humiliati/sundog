...
| 2026-05-22 | **v0.3h ADAPTIVE-FLOOR REPROCESSOR COMPLETE.** `npm run isotrophy:kfacet:reprocess-floor` rewrote the six-row adaptive-floor receipts with D3 isotypic dimensions on the selected kernel. All six resolve without suspicion. Control `O_62` remains `T(2)+S(5)+E(0)` at `1e-8`; the five formerly leaky rows select `k_dim=8` and read `T(2)+S(6)+E(0)`. The repaired boundary vector lands in the sign sector, not the standard sector, so no row requires a Gamma recompute and `c_i=d_i=0` is structurally confirmed for the five conditional rows under the adaptive-floor rule. |
