...
| (next)     | v0.4 chapter opens on `Z_2 = (12)`-equivariant mechanisms for supplementary-B piano-trios, with smaller-symmetry outliers tracked explicitly. Paper-side representation-theory design before any new audit chain or runner code. The methodology surface (sentinel runner, adaptive-floor reprocessor, bridge audit, sigma_3-scan, signed-alignment discipline) carries forward to v0.4; only the group structure changes. |
