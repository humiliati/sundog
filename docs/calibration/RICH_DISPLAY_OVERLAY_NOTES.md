...
| CZA apex → sun altitude / visibility | x: −6.7 px, **y: −19.3 px (−10.6% R₂₂)** | not applicable (h = 59.4° > 32.2° cutoff) | not applicable (cropped; predicted apex y = −50) | **fails coverage gate on current set** | Task #53 step 2 (2026-05-13): parabola-fit apex at (560, 113); predicted (567, 132). x-residual within threshold; **y-residual exceeds 8 px route cutoff by ~2.4×**, but route fails on coverage first (only 1 eligible photo). Measurement is a real receipt of forward/inverse asymmetry per Mesa #3: same photo, parhelion-offset route ~1 px vs CZA-apex route ~19 px. Anchor file: `p2-anchor.json`. |
