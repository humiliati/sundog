...
C2 unrun, the correct handoff language is "unresolved open question" for a
wing-based or Lab b* ridge detector, not a class-level tangent failure.
...
| Tangent-arc curvature -> h | detection gate under current detector | tooling-shape: C2 detector not built |
