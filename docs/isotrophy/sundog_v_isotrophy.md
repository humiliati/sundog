...
| 2026-05-30 | **v0.10/v0.11 FRONTIER ADVANCE LANDED (CONDITIONAL-RANK PROFILE).** The v0.9 reopening avenues were executed in sequence. **v0.10a** (`docs/isotrophy/kfacet/kfacet_v10a_jt_trend_form.md`) replaced the blunt chi^2-of-independence with the phenomenon-aligned one-sided Jonckheere-Terpstra trend test on the frozen v0.9a table and PASSED: exact fixed-margin `p = 7.304e-3` with permutation sanity `p = 7.50e-3`. **v0.10b** (`docs/isotrophy/kfacet/kfacet_v10b_monotone_vf_heldout_predictor_form.md`) tested the stricter mass-marginal held-out predictor and FAILED cleanly: pooled leave-one-m_3-bin-out AUC = `0.4125 <= 0.5`, even though within-bin diagnostics were mostly positive. **v0.11** (`docs/isotrophy/kfacet/kfacet_v11_m3_conditional_vf_rank_form.md`) then registered the localization explicitly: within fixed m_3 strata, the frozen vf zone order ranks stable above unstable on the locked 229-row primary domain, `AUC_cond = 0.6783`, exact stratified `p = 2.046e-7`, exact mass = 1.0, permutation sanity consistent. Interpretation: velocity-fraction is a real in-sample, within-m_3 stability rank signal, but not a mass-marginal held-out predictor; v0.3h K_facet structural-null remains unchanged and no theorem-facing promotion is licensed. Receipts: `results/isotrophy/k-facet-v10a-jt-trend/manifest.json`, `results/isotrophy/k-facet-v10b-monotone-vf-heldout/manifest.json`, `results/isotrophy/k-facet-v11-m3-conditional-vf-rank/manifest.json`. |
| 2026-05-30 | **v0.12 EXTERNAL TRANSFER BLOCKED; v0.13 TARGET SEARCH OPENED.** **v0.12** (`docs/isotrophy/kfacet/kfacet_v12_external_frozen_transfer_form.md`) took the frozen v0.11 rule to same-paper supplementary-A (10,059 orbits; 8,320 candidates after overlap quarantine + strict-m_3=1 exclusion; exact-overlap 235, reflection-overlap 0). The profile cleared all power gates (>= 500 rows, >= 5 strata), but the bounded smoke (30 rows, 69.8 s/row) projected ~161 h single-threaded and ~27% attrition, so the full pass was staged. An unbiased sharded uniform attrition probe (300 rows, 6 shards, seed `20260523`) then read `attrition_fraction = 0.3433`, Wilson95 `[0.2919, 0.3987]` -- the entire CI above the locked 0.20 block gate, systemic across m_3. **Verdict: `external_transfer_blocked_by_attrition`** -- the frozen v0.7 D5 measurement (DOP853, 1e-12) is numerically intractable on ~1/3 of supp-A. This is a measurement-feasibility limit, NOT a falsification of the velocity-fraction hypothesis; the ~2.5 h probe spared the projected 6.7-day run. (Honest correction: the period-stratified smoke at 26.7% under-estimated the uniform rate, not over-estimated.) **v0.13** (`docs/isotrophy/kfacet/kfacet_v13_external_target_search_form.md`; form LOCKED, runner in progress) opened a pre-feature, signal-blind external target-search chapter: inventory + leakage/source-quality gates + D5 feasibility probes (`attrition <= 0.10` AND `Wilson95-upper <= 0.20`) to find and lock an independent (Tier 2/3), low-leakage, D5-tractable external catalog, under a four-layer anti-target-shopping firewall (a target lock is permission to DRAFT a transfer, not to run one). Receipt: `results/isotrophy/k-facet-v12-external-frozen-transfer/`. |
| 2026-06-03 | **v0.13-v0.18 EXTERNAL TRANSFER CAPSTONE LANDED (TIER-2, REPLICATED, MECHANISM-EXPLAINED).** The v0.13 target search closed the independent-target landscape negative: all Tier-3 catalogs failed hard gates, leaving Tier-2 Li/Liao 2021 as the only schema-viable external target. v0.13a bounded leakage at 0.0 but exposed raw vf frame-relativity; v0.13b showed the coarse zone was frame-stable enough to test. v0.14's sampled transfer was `sample_transfer_undecidable_coverage`; v0.15 defeated coverage with stable-support sampling but landed `stable_support_transfer_directional_weak` (`AUC_cond = 0.5125`, p ~3e-4) because the coarse zone saturated. v0.16 read the same projection as a continuous 4-frame ensemble-median vf and landed `tail_resolved_transfer_passes_clean` (`AUC_cond = 0.6469866`, `p = 1e-5`), with the coarse zone on the same rows at 0.510. v0.17 replicated the transfer and the seven-cell heterogeneity on fresh rows (`AUC_cond = 0.646875`, `p = 1e-5`, Spearman vs v0.16 `rho = 1.0`, exact `p_rho = 1/5040`). v0.18 then turned the v0.17 reliability-map interpretation into a registered wider-cell test and landed `reliability_drives_per_cell_auc_supported`: fresh 8 x 8 quadruple-holdout sample (18 cells, 2880 rows, 0 attrition), pooled `AUC_cond = 0.620208`, `p = 1e-5`, label-blind frame-reliability Spearman `rho = 0.5975`, `p = 0.00523`, reversal guard 0 stable-decisive-negative cells. **Interpretation:** the velocity-fraction projection transfers externally when read as a continuous tail-resolved score; its per-cell heterogeneity is explained by frame reliability. Bounds: Tier-2 / Li-Liao lineage / stable-support / within-cell rank / tail-resolved continuous score / not coarse-zone / not full-catalog prevalence / not Tier-3 / not theorem-facing. Receipts: `results/isotrophy/k-facet-v13-external-target-search/`, `results/isotrophy/k-facet-v13b-frame-zone-stability/`, `results/isotrophy/k-facet-v14-liao2021-sampled-transfer/`, `results/isotrophy/k-facet-v15-liao2021-stable-support-transfer/`, `results/isotrophy/k-facet-v16-liao2021-tail-resolved-transfer/`, `results/isotrophy/k-facet-v17-liao2021-heterogeneity/`, `results/isotrophy/k-facet-v18-liao2021-reliability-auc/`. |
| 2026-06-03 | **v0.19 SPECTRAL-GAP RELIABILITY MECHANISM LANDED (`spectral_gap_mechanism_partial`).** Exact v0.18 re-measure over 2,880 rows reproduced `vf`/`score`/`frame_spread` bit-for-bit (`max_abs_diff = 0`). H1 landed strongly and label-blind: small Floquet Re-part gap predicts high frame spread, `Spearman(re_gap, frame_spread) = -0.836`, `p = 1e-5`, with the large-gap/high-spread falsifier nearly empty (15/2880, 0.0052 <= 0.05). H2 missed: per-cell median `gap_reliability` vs v0.18 `AUC_cell` gives `rho = 0.063`, `p = 0.402`. Interpretation: the v0.18 reliability map is spectral at the orbit/fragility level (`spectral gap -> frame spread`; v0.18 already established `frame_spread` tail -> AUC), but the direct median-gap cell aggregation washes out the fragile tail and is not the AUC bridge. Bounds unchanged: Tier-2 / Li-Liao lineage / stable-support / within-cell / tail-resolved continuous score / not theorem-facing; altitude 2 reliability mechanism only, not altitude 1 "why vf -> stability." Receipts: `results/isotrophy/k-facet-v19-spectral-gap-reliability/manifest.json`, `gap_frame_spread_test.json`, `reproduce_v18_check.json`, `docs/isotrophy/kfacet/kfacet_v19_spectral_gap_reliability_form.md`. |
