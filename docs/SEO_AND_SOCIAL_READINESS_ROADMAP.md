...
| `/faraday` | D | minimal (no image) | — | — | draft | draft | — | ✗ | DRAFT staging page filed 2026-05-25 for [`SUNDOG_V_FARADAY.md`](SUNDOG_V_FARADAY.md) v0.1 (Shadow Faraday Zero-Out); Phase 1-4 ledger lives at [`SHADOW_FARADAY.md`](SHADOW_FARADAY.md). Phase 3 landed Branch A in [`FARADAY_PHASE3_DERIVATIONS.md`](FARADAY_PHASE3_DERIVATIONS.md): clean structural zero on the registered classical vacuum domain. Phase 4 landed in [`FARADAY_PHASE4_VERIFICATION.md`](FARADAY_PHASE4_VERIFICATION.md): 5/5 verification/falsification predicates passed. Carries `noindex, nofollow` and a visible DRAFT banner. Held at Class D until Phase 5 chapter close and Bucket 1 readiness land; at that point promote to Class B and clear Bucket 1 (designed 1200×630 og:image, JSON-LD `TechArticle`, tuned title/description, index.html rail card, Post-Inspector pass). Until promoted, **do not** add this URL to the sitemap or share it externally. |
