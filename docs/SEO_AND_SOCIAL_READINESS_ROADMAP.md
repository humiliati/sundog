...
| `/faraday` | D | minimal (no image) | — | — | draft | draft | — | ✗ | DRAFT staging page filed 2026-05-25 for [`SUNDOG_V_FARADAY.md`](SUNDOG_V_FARADAY.md) v0.1 (Shadow Faraday Zero-Out); Phase 1 ledger and Phase 2 operator definition live at [`SHADOW_FARADAY.md`](SHADOW_FARADAY.md), with Phase 2 signed off 2026-05-25 (coordinate plaquette holonomy, two-tier `P_shadow^point` / `P_shadow^stencil` with roles locked, continuum readout as the Phase 3 gate, Floquet/twist deferred); the apparatus card on the page carries the formula. Roadmap Open Questions are also resolved in [`SUNDOG_V_FARADAY.md`](SUNDOG_V_FARADAY.md) (`faraday.html` canonical, Phase 3 strictly source-free + contractible, Phase 4 example slate locked, hand derivation authoritative over SymPy). Carries `noindex, nofollow` and a visible DRAFT banner; Phase 3 zero-out not yet computed. Held at Class D until Phase 3 zero-out lands; at that point promote to Class B and clear Bucket 1 (designed 1200×630 og:image, JSON-LD `TechArticle`, tuned title/description, index.html rail card, Post-Inspector pass). Until promoted, **do not** add this URL to the sitemap or share it externally. |
