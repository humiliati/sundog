...
parameterization-specific. Every 2-DOF map confirms Berry (no A₄, no D₄); the lab found + labelled its
first higher catastrophe and refuted two of its own candidate-A₄ false alarms by code. Still open: the
optional HaloSim cross-render, Phase 7 (forward sweep), Phase 8's pyramidal-capped column, and the
**capstone (Phase 11)**.
