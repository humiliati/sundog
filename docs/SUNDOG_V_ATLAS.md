...
- **Shadow 2 — diffraction (the dressing of the caustic).** Reads crystal **size** — a continuous
  ...
  `atlas/ATLAS_PHASE5_CROSS_SUBSTRATE.md` §3.13): size is *magnitude*-encoded, so ensemble-averaging preserves
...
[`atlas/ATLAS_PHASE5_CROSS_SUBSTRATE.md`](atlas/ATLAS_PHASE5_CROSS_SUBSTRATE.md) — the candidate operator for the
...
substrates) **has now been RUN** (§3.11/§3.13/§3.14 of `atlas/ATLAS_PHASE5_CROSS_SUBSTRATE.md`):
...
| 5 | Determining-shadow-set on the optics — what the display determines vs. resists; bank the halo as a physical read-off resister | advanced — lossiness-crossover RUN (`atlas/ATLAS_PHASE5_CROSS_SUBSTRATE.md` §3.11–3.14): discrete *determines*, continuous magnitude *resists partially* |
...
`atlas/ATLAS_PHASE65_BIFURCATION_SET.md`) — the 22/29/32/46/58 transitions all DERIVED (component-B
...
anywhere is the Lowitz A₃-lips, and two candidate-A₄ false alarms were refuted by code. **Phase 7 DONE
(2026-06-07, `atlas/ATLAS_PHASE7_PHASE_DIAGRAM.md`):** the forward sweep assembled the classified
(elevation × habit) phase diagram — 6 habits, 21 cells, 8 code-reproduced phase-boundary elevations + the
fold-ring radii; the verify pass caught 2 fabricated transitions + 3 catalog corrections. Still open: the
optional HaloSim cross-render, the Phase-7 forward-model gaps (faithful Wegener / random / plate / Parry
generators — none change the classified skeleton), and the **capstone (Phase 11)**.
...
> **REALIZED 2026-06-07 — `docs/atlas/ATLAS_PHASE11_CAPSTONE.md` + `scripts/atlas_model.py`.** The
