...
| --- | --- | --- | --- | --- |
| H1 world/entity-state | high | medium | **LADDER COMPLETE — closed 2026-07-01** | **V3-0 RUN — F3 both corpora** (code `F3-V3/copy`; chess `F3-V3/input`). **V3-0b RUN → `F3-V3b/input`**. **V3-0c RUN → `F3-V3c/witness`** (bank formed; witnesses unavailable at bank scale). **A1 adopted** (order-shuffle control); 29-axis bank `H1-V3-A1-DATA-ADMIT`. **V3-0.5 GPT-2: 0/29, zero pretrained carry.** **V3-1 Qwen2.5-1.5B FULL BANK (GTX-1080 fp16) → `F2-V3c/carry`: 0/29, all surface-blocked (median vs_surface −0.123, best 0.000); vs_randinit median +0.017; shuffle null; d7 whisper regressed to noise.** No live local carry → **H200 definitively not boarded; V3-2 never reached; terminal recommendation = bank R1 + freeze.** `H1_V3_1_ADMISSION_RECEIPT.md` |
