...
| --- | --- | --- |
| Balance | Executable same-shadow floor scaffold. `bayes_floor_shadow_particle` runs with observation parity, no-state-leak, unknown-mode rejection, Phase 10 slate loading, and regret receipts. Post-repair stratified result: 12/12 loaded Phase 10 cells pass Bayes-vs-naive sanity, mean regret versus `sundog_shadow` is +0.0336 normalized survival, and no loaded cell shows negative regret. The candidate diagnostic is mostly guarded parity (`sundog_guard`: 1531 logged rows, `bayes_proposal`: 5), so treat this as a repaired parity floor rather than evidence that explicit Bayes dominates Balance. | [`SUNDOG_V_BALANCE.md` - Bayesian Floor Profile](SUNDOG_V_BALANCE.md#bayesian-floor-profile) and Phase 15 status |
