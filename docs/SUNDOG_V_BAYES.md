...
| --- | --- | --- |
| Balance | Executable same-shadow floor scaffold. `bayes_floor_shadow_particle` runs with observation parity, no-state-leak, unknown-mode rejection, Phase 10 slate loading, and regret receipts. Current stratified result: 12/12 loaded Phase 10 cells pass Bayes-vs-naive sanity, but all still show negative mean regret versus `sundog_shadow`; treat this as a weak-floor repair target, not a Sundog-over-Bayes claim. | [`SUNDOG_V_BALANCE.md` - Bayesian Floor Profile](SUNDOG_V_BALANCE.md#bayesian-floor-profile) and Phase 15 status |
