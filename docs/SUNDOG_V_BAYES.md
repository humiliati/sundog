...
| --- | --- | --- |
| Balance | Executable same-shadow floor scaffold. `bayes_floor_shadow_particle` runs with observation parity, no-state-leak, unknown-mode rejection, Phase 10 slate loading, and regret receipts. The 12-cell post-repair probe passed, but the 24-cell loader probe exposed a sensor-noise block: 23/24 cells passed sanity, one recoverable `noise_0p03` cell was negative versus `sundog_shadow`, and a focused sensor-noise panel passed only 6/10 sanity cells. Treat Balance as a guarded parity floor with a pending sensor-noise admission/guard repair before any full claim lock. | [`SUNDOG_V_BALANCE.md` - Bayesian Floor Profile](SUNDOG_V_BALANCE.md#bayesian-floor-profile) and Phase 15 status |
