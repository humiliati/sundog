...
| --- | --- | --- |
| Balance | Executable same-shadow floor scaffold. `bayes_floor_shadow_particle` runs with observation parity, no-state-leak, unknown-mode rejection, Phase 10 slate loading, and regret receipts. The full diagnostic lock ran 27,200 trials with audits passing, aggregate regret versus `sundog_shadow` +0.02645, and 59/68 Bayes-vs-naive sanity cells passing. Treat Balance as a guarded parity floor with a pending observation-degradation admission/guard repair: the remaining failures cluster on long sensor delay and sensor noise, with two negative mean-regret cells. | [`SUNDOG_V_BALANCE.md` - Bayesian Floor Profile](SUNDOG_V_BALANCE.md#bayesian-floor-profile) and Phase 15 status |
