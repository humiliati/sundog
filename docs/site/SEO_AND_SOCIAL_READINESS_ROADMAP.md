...
| `/isotrophy` | A | ✓ | isotrophy.png | TechArticle | ✓ | ✓ | sitemap + homepage pillar | ✓ | filed 2026-05-22 with full Class A treatment; broadened 2026-05-25 from the v0.3h public companion to the v0.3-v0.9 isotrophy audit-chain pause page, refreshed 2026-05-30 for the v0.10/v0.11 conditional-rank profile, 2026-06-02 for the v0.12-v0.16 external-transfer arc, then 2026-06-03 for the v0.17 heterogeneity replication. Viz remains the v0.3h 21-cell catalog grid (20 gold structural-zero receipts + 1 dashed-red quarantine for O_617), which is still the first load-bearing pillar. Current copy carries the v0.7 velocity-fraction positive, v0.10b mass-marginal held-out null, v0.11 within-m3 conditional-rank pass, v0.12 supplementary-A attrition block, v0.16 bounded Tier-2 external confirmation on liao2021 stable-support rows after tail-resolved continuous scoring, and v0.17 fresh-row replication of the pooled transfer plus seven-cell heterogeneity ordering. Sister ledger at [`SUNDOG_V_ISOTROPHY_KFACET.md`](../isotrophy/SUNDOG_V_ISOTROPHY_KFACET.md). |
...
| `/kakeya` | D | — | — | — | — | — | — | — | Review-only finite-field Kakeya reader surface, registered 2026-06-01 to unblock the build (was an unfiled root HTML page failing the vite manifest check). `noindex, nofollow` + a visible `NOT PEER REVIEWED` banner; **no public inbound path** (not in nav/index/legend) and excluded from the sitemap until an external finite-field-incidence sanity check clears it (Bucket 1 + sign-off). Interactive `workbench.html`/`gallery.html` deployed unlinked under `/kakeya/` via `scripts/copy-site-docs.mjs`. Reviewer packet: [`EXTERNAL_REVIEW_PACKET.md`](../kakeya/EXTERNAL_REVIEW_PACKET.md). Promote to Class A/B only on authoritative sign-off. |
| `/boxsel` | D | — | — | — | — | — | — | — | Phase-8 BoxSEL false-closure workbench, registered 2026-06-21 as a review-only internal demonstrator after the locked Phase-7b toy micro-corpus pass. `noindex, nofollow`; no public inbound path; excluded from sitemap until a separate public-copy gate and toy-boundary sign-off promote it. Data is generated from `results/boxsel/phase7b_false_closure_run/manifest.json` by `scripts/boxsel_phase8_workbench_data.py`; result note: [`PHASE7B_FALSE_CLOSURE_RUN.md`](../boxsel/PHASE7B_FALSE_CLOSURE_RUN.md). |
...
| `/generality` | A | ✓ | generality.png | TechArticle | ✓ | ✓ | homepage evidence grid + `/h-of-x` + geometry/alignment/legend/repo-map + sitemap | ✓ | Filed 2026-05-30 as the umbrella over six high-stakes transfer lanes: NSE C1 (review-gated witness, strongest current lane — links out to [`/navierstokes`](navierstokes) for the deep dive), Riemann (3 lanes, 3 bounded nulls, review-gated), Yang-Mills (4 probes, 1 bounded null, Phase 2 NEG-A across the slate), P-vs-NP (v6 bounded-positive op-count certificate plus Lean-checked soundness/lossiness; hardness imported; public Lean method repo now spans finite-field algebra, real analysis, and geometric optics), ARC-AGI (execution-hold after sparse absolute fibers and oracle-leakage findings), Three-Body 15C (running, 6/12 shards logged). Bespoke 1200×630 `og:image` shows a 6-tile mini-matrix with NSE prominent (bolder border) and P-vs-NP differentiated. Carries the asymmetric framing explicitly — NSE has its own page, the other five are honest cards on the matrix — and now includes a site-route bridge from `/h-of-x` first-equation discipline to harder transfer-test substrates. Four claim-boundary disavowals: not a Clay-problem claim, not a theorem-claims page, not symmetric weight, not a launch surface for the underlying ledgers. Post-deploy LinkedIn/Twitter validator pass remains to record after deployment. |
| `/tauroctony` | A | ✓ | tauroctony.png | Article | ✓ | ✓ | brand-essay surface; ledger backer at `SUNDOG_V_TAUROCTONY.md`; LW/AF technical companion at `mesa/CAP_NOT_COUNCIL_LW_POST.md`; brand essay source at `mesa/CREDIT_THE_CAP_NOT_THE_COUNCIL_POST.md`; inbound from Mesa lane + sitemap | ✓ | Filed 2026-06-30 as the public brand surface for the Tauroctony cosmogram ledger's deflation receipt. Hero with Diablo II Mithraeum header image; story arc from the pantheon scaffold to one inequality (`Sov_opt ≤ κ`); four-axis result table (competence parity, corrigibility via override, non-sovereignty p95 0.30 vs 0.71, safe-interruptibility `band_avoidance ≤ 0.13` at every `κ ∈ {0.4 … 1.0}`); NS-3 replication under a learned presider; NS-4 competent-sandbagger elicited then deterred (`CAP_DETERS_COMPETENT_SANDBAG`, geometric check on the elicited policy, not a fresh training run); three machine-checked Lean lemmas (`optimum_mono`, `signature_noninterference` axiom-free, `ruin_break_even`); Monet-style oil-painting tauroctony in the cosmogram section; released task family at `released/non-sovereignty/`, Apache-2.0. Bespoke 1200×630 `og:image` rendered 2026-06-30 via `_generate.py` pipeline (`viz_tauroctony` — the pantheon → `Sov_opt ≤ κ` deflation arc + four-axis pills + axiom-clean/released-benchmark footer). Standing claim boundary above the fold: Mithraic framing is **ornament throughout, not evidence**; operating-envelope evidence not proof; does not transfer to foundation models; field-grounding **relocates** the attack surface (not deletes; `signature_noninterference` is the receipt); Small/Medium in-vitro task family; out of scope: adversaries that tamper with the presider's *inputs*. The Ornament Rule held throughout: every load-bearing claim was already routed to a typed home with a named falsifier, so when the deflation landed there was nothing to retract. Owner-gated deploy; post-deploy LinkedIn/Twitter validator pass remains to record. Companion artifacts: [`SUNDOG_V_TAUROCTONY.md`](../SUNDOG_V_TAUROCTONY.md) (ledger, now DELIVERED), [`mesa/CREDIT_THE_CAP_NOT_THE_COUNCIL_POST.md`](../mesa/CREDIT_THE_CAP_NOT_THE_COUNCIL_POST.md) (brand essay), [`mesa/CAP_NOT_COUNCIL_LW_POST.md`](../mesa/CAP_NOT_COUNCIL_LW_POST.md) (LW/AF technical companion — brand-stripped). |
| `/quantilizing` | A | ✓ (page) / — (image pending) | quantilizing.png (pending) | WebPage | ✓ | ✓ | sitemap + `site-pages.json`; catalog cross-refs from `SUNDOG_V_PERCIVAL.md` and Percival ledger surfaces (docs/index.html, docs/README.md, repo-map.html, tauroctony.html) | ✓ | **STAGED 2026-07-01, PRE-DEPLOY.** Angle 1 (counterproductivity theorem) surface off [`PERCIVAL_PROMO_HANDOFF.md`](../percival/PERCIVAL_PROMO_HANDOFF.md). De-Percivaled title ("When Quantilizing Hurts") — no Grail/romance framing on-page. Payload: on rewards nonincreasing in proxy-pressure (proxy pointwise misaligned), quantilizing cannot improve true reward; the best `q`-quantilizer is the untilted base (`q=1`); iterated tilting drifts to ruin; a zero-causal-proxy-dependence policy strictly out-collects the family when the base's support is past the collapse threshold, partially otherwise. Machine-checked in Lean 4 + Mathlib for arbitrary finite uniform support (`upper_tail_mul_le_base_mul`, `best_quantilizer_is_base_general`, `clean_support_above_separation_general`; axioms `[propext, Classical.choice, Quot.sound]`, no `sorry`), fuzz-tested on 10,500 instances including the actual coordination-court model. Boundary table separates the theorem class (misaligned-or-neutral + any monotone tax including none) from the alternate route to `q*=1` (pointwise-good + graded-and-steep tax). Corollary: threshold oversight *creates* cliff-edge temptation; graded oversight *removes* it. Cites Taylor 2015, Perdomo et al. 2020, Elster 1983, Manheim-Garrabrant 2018, Carlsson-van Damme 1993, Morris-Shin 1998. **Hard fences** honored (per `PERCIVAL_PROMO_HANDOFF.md` §HARD FENCES, all nine): no foundation-model claim; condition "pointwise misaligned" carried on every statement; no measured-real-base assertion; clean unconditional separation NOT made and is reported as central finding rather than caveat; Lean scope explicit (finite uniform; weighted/continuous computed-only); multi-agent case is *worse*, coalition-required; medievalist branch gated (no Grail-corpus framing surfaced); three standing falsifiers visible in §5 fence card. **Pre-deploy owed** (per HTML comment + `site-pages.json` `publicLaunchIntent`): (1) owner re-voice on both the HTML and companion note `docs/percival/WHEN_QUANTILIZING_HURTS_NOTE.md`; (2) owner licensing call — root LICENSE is UNLICENSED, per-file grant / release manifest decision undecided; (3) OG image generation via `viz_quantilizing` CARDS entry in `public/og/_generate.py` (staged in this same commit; image not yet produced). Post-deploy LinkedIn/Twitter validator pass remains to record after deployment. Companion source: [`WHEN_QUANTILIZING_HURTS_NOTE.md`](../percival/WHEN_QUANTILIZING_HURTS_NOTE.md); promo handoff: [`PERCIVAL_PROMO_HANDOFF.md`](../percival/PERCIVAL_PROMO_HANDOFF.md) (per-file Apache-2.0 grant). |
| `/algorithmic-approximation` | A | ✓ | algorithmic-approximation.png | TechArticle | ✓ | ✓ | sibling to `/p-vs-np` + inbound lane card from `/generality` umbrella + homepage pillar action link + sitemap | ✓ | Filed 2026-06-29 as the public surface for the algorithmic-approximation lane (off arXiv:2606.26705 Kratsios et al.). Leads with the find/check ledger (`Certifies` + seven cheap-CHECK instances under one interface: max-flow/min-cut, König matching/cover, 2-SAT decision, Pratt primality, shortest-path, syndrome verifier, ReLU gate-count) — each machine-checks an `O(·)` cheap-CHECK theorem while finding stays imported in every instance. Alongside, the tropical / PL core machine-checks exact (`ε = 0`) compilation of cancellation-free PL circuits to ReLU networks with **linear gate count `≤ 4N` via a sharing-aware DAG** (`compileToDag` — closes the prior linear-gate-count caveat), realization-independent linear-region structure, monotone vs cancellation distinguishing, and the tent's `2^d` depth separation — the `ε = 0` PL core for arXiv:2606.26705 Thm 3.2 / Cor 5.1. N-1 closes monotone depth-as-computation (cancellation-free circuits region-polynomial). Honest secondary tier: N-2 cancellation-spine is a typed-conjecture organizing read (machine-checked on monotone/convex/region only); N-4 ε-essential region count is empirical (0.89 tracking, named SGD residual). Bespoke 1200×630 `og:image` rendered 2026-06-29 via the `_generate.py` pipeline (`viz_algo_approx_ledger` — 7-instance ledger table with cheap-CHECK pills, axiom-clean badge). Six explicit do-not-say disavowals above the fold (NOT P-vs-NP advance, NOT hardness proof, NOT learnability claim, NOT full analytic-gate approximation theorem, NOT fast primality test, NOT general neural-net lower bound) and six named falsifiers across the result set. Receipts: [`ALGO_APPROX_CONJECTURE_SLATE_2.md`](../ALGO_APPROX_CONJECTURE_SLATE_2.md), [`ALGO_APPROX_N2_CANCELLATION_SPINE.md`](../ALGO_APPROX_N2_CANCELLATION_SPINE.md), [`ALGO_APPROX_N4_EPS_REGIONS_RESULT.md`](../ALGO_APPROX_N4_EPS_REGIONS_RESULT.md), [`SUNDOG_V_CERTIFICATE_LEAN.md`](../SUNDOG_V_CERTIFICATE_LEAN.md), [`SUNDOG_V_ALGO_APPROX.md`](../algo-approx/SUNDOG_V_ALGO_APPROX.md). Post-deploy LinkedIn/Twitter validator pass remains to record after deployment. |
