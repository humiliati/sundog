...
| Tangent-arc curvature → h | **not promoted** | coverage gate fails (1/3 photos) and the C5 manual recovery on p2 doesn't survive two complementary falsifications: C6 matched-filter on the b* substrate (project-original handle) and C7 reformulation under the canonical literature handle (opening angle / arc extent per Tape 1994 Ch 6, tested against HaloSim ray-tracing). Under the canonical handle, C5's circle fit places wings 1.2-5.6° too-far-radially. The C5↔C6 tension resolves: C5 was likely symmetry-bias artifact, not a genuine recovery. |
...
- Photo-data privacy policy: [`PHOTO_DATA_POLICY.md`](../site/PHOTO_DATA_POLICY.md)
...
| 6 | Upper / Lower Tangent Arc tangent to 22° halo top/bottom | "Eyelid" geometry from column-oriented crystals. The literature-standard *inverse* handle for tangent arcs is opening angle / arc extent (Tape 1994 Ch 6; Cowley tangent-arcs page), not circle-fit curvature. The project tested both handles in 2026-05-14: circle-fit (Pass C5/C6) and opening-angle (Pass C7 against HaloSim ray-tracing). Neither handle promotes the tangent route on the current calibration set. | Greenler ch. 3 |
