#!/usr/bin/env python
"""ARC Phase 3E signature-fiber certificate runner (phase3e_signature_fiber_certificate).

Standalone certificate runner. Does NOT train a grid decoder. It computes the
geometry of `signature_palette_context` fibers over the registered Phase 0
contexts and certifies whether exact / near collisions with incompatible
required behavior exist.

The arc-p3-feature-v1 grid encoders, baseline family, mask candidate bank, and
edit-color rule bank are inherited verbatim from the frozen Branch D mask-target
runner (with header markers) and reused ONLY to compute the `program_sketch_v1`
oracle audit labels. The certificate's own context identity, context distance,
collision detection, k=3 cross-task fiber locality, and 5-branch precedence
adjudication are implemented in the "Certificate" section at the bottom.

The program-sketch oracle banks run with a fixed representation-neutral grid
arm (`raw_grid_edit_mask_v3`) so the behavior labels reflect true grids, not a
signature projection. The certificate ARMS (signature_palette_context, etc.)
are used only for context identity + distance.

Spec: docs/prereg/arc/PHASE3E_SIGNATURE_FIBER_CERTIFICATE_SPEC.md (filed 2026-05-28).
Prior Branch D mask-target spec: docs/prereg/arc/PHASE3D_MASK_TARGET_VARIANT_SPEC.md.
...
LEARNER_VERSION = "signature_fiber_certificate_v1"
PROTOCOL_VERSION = "arc-p3e-signature-fiber-certificate-v1"
RECEIPT_SCHEMA_VERSION = "arc-p3e-signature-fiber-certificate-receipt-v1"
