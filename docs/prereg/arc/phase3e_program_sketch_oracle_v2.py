#!/usr/bin/env python
"""ARC Phase 3E program-sketch oracle v2 runner (phase3e_program_sketch_oracle_v2).

Standalone certificate-labeler runner. It is NOT a decoder and NOT a Branch E
solver. It replaces the prior-blind `program_sketch_v1` (built from the Branch D
edit-composition banks) with a framing-agnostic `program_sketch_v2` oracle: nine
deterministic transformation-relation facets computed from RAW registered grids,
gated by anti-vacuity / anti-prior-laundering / anti-solver-leakage tests, then
re-run through the FROZEN Phase 3E signature-fiber geometry.
...
Spec: docs/prereg/arc/PHASE3E_PROGRAM_SKETCH_ORACLE_V2_SPEC.md (filed 2026-05-29).
...
LEARNER_VERSION = "program_sketch_oracle_v2"
PROTOCOL_VERSION = "arc-p3e-program-sketch-oracle-v2"
RECEIPT_SCHEMA_VERSION = "arc-p3e-program-sketch-oracle-v2-receipt-v1"
...
# ============================================================================
# program_sketch_v2: main + artifacts
# ============================================================================
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ARC Phase 3E program-sketch oracle v2")
...
V2_FILES_EMPTY = {
    ...
    "program_sketch_v2_summary.csv": ["context_id", "lane", "task_id", "prior", "nonempty_facets", "vacuous", "extra_facets_outside_prior"],
    "prior_laundering_audit.csv": ["context_id", "prior", "extra_facets_outside_prior", "violates_two_extra_rule"],
    "solver_leakage_audit.csv": ["metric", "value", "threshold", "pass"],
    "oracle_coverage_by_prior.csv": ["primary_prior", "n_contexts", "n_vacuous", "vacuous_fraction", "test_lodo_has_nonvacuous", "pttest_has_nonvacuous"],
    "pairwise_top_neighbors_v2.csv": ["context_id", "lane", "task_id", "neighbor_context_id", "neighbor_task_id", "distance", "cross_task", "fidelity_pass"],
    "near_fiber_incompatibilities_v2.csv": ["context_a", "task_a", "context_b", "task_b", "distance", "n_disjoint_facets", "hard_incompat", "facet_jaccard"],
    "knn_fiber_locality_v2.csv": ["context_id", "lane", "task_id", "prior", "fidelity_pass", "r_k", "local_incompatibility_rate", "same_prior_neighbor_rate", "n_cross_task_neighbors"],
}


def write_empty_v2_receipt(out_dir: Path, manifest: dict[str, Any]) -> None:
    write_json(out_dir / "manifest.json", manifest)
    for fname, cols in V2_FILES_EMPTY.items():
    ...
    write_json(out_dir / "phase3e_program_sketch_oracle_v2_receipt.json", {"manifest": manifest, "branch": None})
    (out_dir / "branch_adjudication.md").write_text("# Phase 3E Program-Sketch Oracle v2\n\nDry run / empty receipt.\n", encoding="utf-8")
    (out_dir / "commands.md").write_text("# Phase 3E v2 commands\n\nDry run / empty receipt.\n", encoding="utf-8")
...
def main() -> int:
    ...
    spec_path = Path(__file__).resolve().parent / "PHASE3E_PROGRAM_SKETCH_ORACLE_V2_SPEC.md"
    parent_spec_path = Path(__file__).resolve().parent / "PHASE3E_SIGNATURE_FIBER_CERTIFICATE_SPEC.md"
    ...
    manifest: dict[str, Any] = {
        ...
        "tool": "docs/prereg/arc/phase3e_program_sketch_oracle_v2.py",
        "command": [sys.executable, "docs/prereg/arc/phase3e_program_sketch_oracle_v2.py", *sys.argv[1:]],
        ...
        "specPath": "docs/prereg/arc/PHASE3E_PROGRAM_SKETCH_ORACLE_V2_SPEC.md", "specHash": spec_hash,
        "parentSpecPath": "docs/prereg/arc/PHASE3E_SIGNATURE_FIBER_CERTIFICATE_SPEC.md", "parentSpecHash": parent_spec_hash,
        "runnerSha256": self_hash, "pythonVersion": sys.version, "platform": platform.platform(),
        "certArms": CERT_ARMS, "primaryCertArm": PRIMARY_CERT_ARM,
        "sketchFacets": SKETCH_FACETS,
        "thresholds": {
            "epsilon_primary": EPSILON_PRIMARY, "epsilon_strict": EPSILON_STRICT,
            "epsilon_loose": EPSILON_LOOSE, "knn_k": KNN_K,
            "vacuity_min_nonempty_facets": VACUITY_MIN_NONEMPTY_FACETS,
            "vacuity_overall_max": VACUITY_OVERALL_MAX, "vacuity_per_prior_max": VACUITY_PER_PRIOR_MAX,
            "laundering_min_extra_facets": LAUNDERING_MIN_EXTRA_FACETS,
            "laundering_max_violation_fraction": LAUNDERING_MAX_VIOLATION_FRACTION,
            "leakage_unique_core_max": LEAKAGE_UNIQUE_CORE_MAX, "leakage_exact_lookup_max": LEAKAGE_EXACT_LOOKUP_MAX,
            "v2_min_disjoint_facets": V2_MIN_DISJOINT_FACETS,
        },
    }

    if args.dry_run:
        manifest["mode"] = "dry_run"; manifest["completedAt"] = iso_now()
        write_empty_v2_receipt(out_dir, manifest)
        print(f"ARC Phase 3E v2 oracle dry run wrote {out_dir}")
        return 0
    ...
    write_csv(out_dir / "split.csv", [{"task_id": t.task_id, "primary_prior": t.primary_prior, "predicted_boundary": t.predicted_boundary, "split": t.split} for t in sorted(tasks, key=lambda x: x.task_id)], V2_FILES_EMPTY["split.csv"])
    ...
    # Stage 2: v2 sketches (post-barrier)
    sketches: dict[str, dict[str, Any]] = {}
    sketch_jsonl = []
    for c in u_all:
        sk = program_sketch_v2(c)
        ...
        c["_target_exact_hash"] = target_exact_hash(c["target_output"])
        c["_target_sp_id"] = target_signature_palette_id(c["target_output"])
        sketch_jsonl.append({"instance_id": c["instance_id"], "lane": c["lane"], "task_id": c["task_id"], "prior": c["primary_prior"], **sk})
    write_jsonl(out_dir / "program_sketch_v2.jsonl", sketch_jsonl)
    ...
    laundering_rows = []
    coverage_rows = []
    n_vacuous = sum(1 for c in primary if is_vacuous(sketches[c["instance_id"]]))
    vacuous_fraction = n_vacuous / len(primary) if primary else 0.0
    priors = sorted({c["primary_prior"] for c in primary})
    per_prior_vac_ok = True
    per_prior_lane_ok = True
    for prior in priors:
        ctxs = [c for c in primary if c["primary_prior"] == prior]
        vac = [c for c in ctxs if is_vacuous(sketches[c["instance_id"]])]
        frac = len(vac) / len(ctxs) if ctxs else 0.0
        if frac > VACUITY_PER_PRIOR_MAX:
            per_prior_vac_ok = False
        tl = [c for c in ctxs if c["lane"] == "test_lodo"]
        pt = [c for c in ctxs if c["lane"] == "pttest"]
        tl_nonvac = any(not is_vacuous(sketches[c["instance_id"]]) for c in tl)
        pt_nonvac = any(not is_vacuous(sketches[c["instance_id"]]) for c in pt)
        if tl and not tl_nonvac:
            per_prior_lane_ok = False
        if pt and not pt_nonvac:
            per_prior_lane_ok = False
        coverage_rows.append({"primary_prior": prior, "n_contexts": len(ctxs), "n_vacuous": len(vac), "vacuous_fraction": round_float(frac), "test_lodo_has_nonvacuous": tl_nonvac if tl else "n/a", "pttest_has_nonvacuous": pt_nonvac if pt else "n/a"})
    write_csv(out_dir / "oracle_coverage_by_prior.csv", coverage_rows, V2_FILES_EMPTY["oracle_coverage_by_prior.csv"])
    vacuity_gate_pass = (vacuous_fraction <= VACUITY_OVERALL_MAX) and per_prior_vac_ok and per_prior_lane_ok

    # ---- Gate 2: anti-prior-laundering ----
    n_laundering_violations = 0
    n_nonvacuous = 0
    for c in primary:
        sk = sketches[c["instance_id"]]
        vac = is_vacuous(sk)
        extra = laundering_extra_facet_count(sk, c["primary_prior"])
        violates = (not vac) and (extra < LAUNDERING_MIN_EXTRA_FACETS)
        if not vac:
            n_nonvacuous += 1
            if violates:
                n_laundering_violations += 1
        laundering_rows.append({"context_id": c["instance_id"], "prior": c["primary_prior"], "extra_facets_outside_prior": extra, "violates_two_extra_rule": violates})
        sk_summary.append({"context_id": c["instance_id"], "lane": c["lane"], "task_id": c["task_id"], "prior": c["primary_prior"], "nonempty_facets": nonempty_facet_count(sk), "vacuous": vac, "extra_facets_outside_prior": extra})
    write_csv(out_dir / "prior_laundering_audit.csv", laundering_rows, V2_FILES_EMPTY["prior_laundering_audit.csv"])
    write_csv(out_dir / "program_sketch_v2_summary.csv", sk_summary, V2_FILES_EMPTY["program_sketch_v2_summary.csv"])
    laundering_violation_fraction = (n_laundering_violations / n_nonvacuous) if n_nonvacuous else 0.0
    laundering_gate_pass = laundering_violation_fraction <= LAUNDERING_MAX_VIOLATION_FRACTION

    # ---- Gate 3: anti-solver-leakage ----
    syntactic_fails: list[str] = []
    for c in u_all:
        syntactic_fails += _syntactic_leakage(sketches[c["instance_id"]])
    core_tuples = [core_sketch_tuple(sketches[c["instance_id"]]) for c in primary]
    unique_core_fraction = (len(set(core_tuples)) / len(core_tuples)) if core_tuples else 0.0
    # exact-output lookup keyed by core_sketch: group primary by core tuple; a group
    # "solves" a context if all members share the same target_exact_hash (a lookup
    # keyed only by core_sketch would return the right output).
    from collections import defaultdict as _dd
    core_groups: dict[tuple, list[dict[str, Any]]] = _dd(list)
    for c in primary:
        core_groups[core_sketch_tuple(sketches[c["instance_id"]])].append(c)
    solvable = 0
    for grp in core_groups.values():
        hashes = {m["_target_exact_hash"] for m in grp}
        if len(hashes) == 1:  # core_sketch maps to a unique output -> lookup would solve all members
            solvable += len(grp)
    exact_lookup_fraction = (solvable / len(primary)) if primary else 0.0
    leakage_stat_fail = (unique_core_fraction > LEAKAGE_UNIQUE_CORE_MAX) or (exact_lookup_fraction > LEAKAGE_EXACT_LOOKUP_MAX)
    leakage_gate_pass = (not syntactic_fails) and (not leakage_stat_fail)
    write_csv(out_dir / "solver_leakage_audit.csv", [
        {"metric": "syntactic_failures", "value": ";".join(sorted(set(syntactic_fails))) or "none", "threshold": "0", "pass": not syntactic_fails},
        {"metric": "unique_core_sketch_fraction", "value": round_float(unique_core_fraction), "threshold": LEAKAGE_UNIQUE_CORE_MAX, "pass": unique_core_fraction <= LEAKAGE_UNIQUE_CORE_MAX},
        {"metric": "core_sketch_exact_lookup_fraction", "value": round_float(exact_lookup_fraction), "threshold": LEAKAGE_EXACT_LOOKUP_MAX, "pass": exact_lookup_fraction <= LEAKAGE_EXACT_LOOKUP_MAX},
    ], V2_FILES_EMPTY["solver_leakage_audit.csv"])

    # ---- Fiber rerun (frozen geometry) ----
    # exact / representation-level collisions over U_primary
    from collections import defaultdict as _dd2
    groups: dict[str, list[dict[str, Any]]] = _dd2(list)
    for c in u_primary:
        groups[c["_context_id_sp"]].append(c)
    exact_found = rep_found = False
    for cid, members in groups.items():
        if len(members) < 2:
            continue
        if len({m["_target_exact_hash"] for m in members}) > 1:
            exact_found = True
        if len({m["_target_sp_id"] for m in members}) > 1:
            rep_found = True
    ...
    for i in range(len(u_primary)):
        for j in range(i + 1, len(u_primary)):
            ...
            if d > EPSILON_PRIMARY:
                continue
            incompat, n_disj = v2_incompatible(sketches[ca["instance_id"]], sketches[cb["instance_id"]])
            hard = _hard_incompat(sketches[ca["instance_id"]], sketches[cb["instance_id"]])
            jac = _v2_facet_jaccard(sketches[ca["instance_id"]], sketches[cb["instance_id"]])
            if incompat:
                near_incompat_found = True
            near_rows.append({"context_a": ca["instance_id"], "task_a": ca["task_id"], "context_b": cb["instance_id"], "task_b": cb["task_id"], "distance": d, "n_disjoint_facets": n_disj, "hard_incompat": hard, "facet_jaccard": round_float(jac)})
    write_csv(out_dir / "near_fiber_incompatibilities_v2.csv", [r for r in near_rows if r["n_disjoint_facets"] >= V2_MIN_DISJOINT_FACETS or r["hard_incompat"]], V2_FILES_EMPTY["near_fiber_incompatibilities_v2.csv"])
    ...
    local_rates = []
    for c in u_primary:
        dists = sorted(((context_distance("signature_palette_context", c, o), o) for o in u_primary if o["instance_id"] != c["instance_id"] and o["task_id"] != c["task_id"]), key=lambda t: (t[0], t[1]["instance_id"]))
        knn = dists[:KNN_K]
        for d, nb in knn:
            top_rows.append({"context_id": c["instance_id"], "lane": c["lane"], "task_id": c["task_id"], "neighbor_context_id": nb["instance_id"], "neighbor_task_id": nb["task_id"], "distance": d, "cross_task": True, "fidelity_pass": d <= EPSILON_PRIMARY})
        r_k = knn[-1][0] if knn else 1.0
        fpass = [(d, nb) for d, nb in knn if d <= EPSILON_PRIMARY]
        fidelity = len(fpass) >= 1
        if fidelity:
            n_fidelity_pass += 1
        inc = sum(1 for d, nb in fpass if v2_incompatible(sketches[c["instance_id"]], sketches[nb["instance_id"]])[0])
        same_prior = sum(1 for d, nb in fpass if nb["primary_prior"] == c["primary_prior"])
        local = (inc / len(fpass)) if fpass else 0.0
        if fidelity:
            local_rates.append(local)
        knn_rows.append({"context_id": c["instance_id"], "lane": c["lane"], "task_id": c["task_id"], "prior": c["primary_prior"], "fidelity_pass": fidelity, "r_k": round_float(r_k), "local_incompatibility_rate": round_float(local), "same_prior_neighbor_rate": round_float((same_prior / len(fpass)) if fpass else 0.0), "n_cross_task_neighbors": len(knn)})
    write_csv(out_dir / "knn_fiber_locality_v2.csv", knn_rows, V2_FILES_EMPTY["knn_fiber_locality_v2.csv"])
    write_csv(out_dir / "pairwise_top_neighbors_v2.csv", top_rows, V2_FILES_EMPTY["pairwise_top_neighbors_v2.csv"])

    fidelity_fraction = (n_fidelity_pass / len(u_primary)) if u_primary else 0.0
    mean_local = (sum(local_rates) / len(local_rates)) if local_rates else 0.0

    # ---- 7-branch precedence ----
    if not leakage_gate_pass:
        branch = "phase3e_oracle_v2_solver_leakage"
        reason = "Anti-solver-leakage gate failed (syntactic or statistical); the oracle is too specific."
    elif not laundering_gate_pass:
        branch = "phase3e_oracle_v2_prior_laundering"
        reason = f"Anti-prior-laundering gate failed: {laundering_violation_fraction:.1%} of non-vacuous primary contexts violate the two-extra-facet rule (> {LAUNDERING_MAX_VIOLATION_FRACTION:.0%})."
    elif not vacuity_gate_pass:
        branch = "phase3e_oracle_v2_vacuous"
        reason = f"Anti-vacuity gate failed (overall vacuous fraction {vacuous_fraction:.1%}, threshold {VACUITY_OVERALL_MAX:.0%}; per-prior or per-lane coverage unmet)."
    elif exact_found or rep_found:
        branch = "phase3e_v2_exact_fiber_collision"
        reason = "Exact or representation-level signature_palette fiber collision exists under unchanged context identity."
    elif near_incompat_found:
        branch = "phase3e_v2_near_fiber_incompatibility"
        reason = "A cross-task near-fiber pair at epsilon_primary is incompatible under program_sketch_v2."
    elif fidelity_fraction >= 0.50 and mean_local <= 0.10:
        branch = "phase3e_v2_fiber_locality_positive"
        reason = f"Gates pass; {fidelity_fraction:.1%} of primary contexts have fidelity-passing cross-task neighborhoods and mean local incompatibility {mean_local:.3f} <= 0.10."
    else:
        branch = "phase3e_v2_deferred_sparse_fibers"
        reason = f"Gates pass, but only {fidelity_fraction:.1%} of primary contexts have fidelity-passing cross-task neighborhoods (< 50%); the registered universe is too sparse at the frozen radius."

    manifest["completedAt"] = iso_now()
    manifest["branch"] = branch
    manifest["branchReason"] = reason
    manifest["gates"] = {
        "vacuity_gate_pass": vacuity_gate_pass, "vacuous_fraction": round_float(vacuous_fraction),
        "laundering_gate_pass": laundering_gate_pass, "laundering_violation_fraction": round_float(laundering_violation_fraction),
        "leakage_gate_pass": leakage_gate_pass, "unique_core_sketch_fraction": round_float(unique_core_fraction),
        "core_sketch_exact_lookup_fraction": round_float(exact_lookup_fraction),
        "syntactic_leakage_failures": sorted(set(syntactic_fails)),
    }
    manifest["fiber"] = {
        "exact_collision_found": exact_found, "rep_collision_found": rep_found,
        "near_incompat_found": near_incompat_found,
        "n_near_pairs_within_epsilon": len(near_rows),
        "n_near_incompatibilities": len([r for r in near_rows if r["n_disjoint_facets"] >= V2_MIN_DISJOINT_FACETS or r["hard_incompat"]]),
        "fidelity_pass_fraction": round_float(fidelity_fraction), "mean_local_incompatibility": round_float(mean_local),
        "min_cross_task_distance": round_float(min((r["distance"] for r in top_rows), default=1.0)),
    }

    write_json(out_dir / "manifest.json", manifest)
    write_json(out_dir / "phase3e_program_sketch_oracle_v2_receipt.json", {"manifest": manifest, "branch": branch, "branchReason": reason, "coverageByPrior": coverage_rows})
    lines = [
        "# Phase 3E Program-Sketch Oracle v2", "",
        f"Branch: **{branch}**", "", reason, "",
        f"- U_primary contexts: {len(u_primary)} (U_all: {len(u_all)})",
        f"- vacuity gate: {'PASS' if vacuity_gate_pass else 'FAIL'} (vacuous fraction {vacuous_fraction:.1%}, threshold {VACUITY_OVERALL_MAX:.0%})",
        f"- prior-laundering gate: {'PASS' if laundering_gate_pass else 'FAIL'} (violation fraction {laundering_violation_fraction:.1%})",
        f"- solver-leakage gate: {'PASS' if leakage_gate_pass else 'FAIL'} (unique_core {unique_core_fraction:.2f}, exact_lookup {exact_lookup_fraction:.2f})",
        f"- exact/representation collisions: {exact_found or rep_found}",
        f"- near pairs within epsilon_primary={EPSILON_PRIMARY}: {len(near_rows)}; incompatible: {len([r for r in near_rows if r['n_disjoint_facets'] >= V2_MIN_DISJOINT_FACETS or r['hard_incompat']])}",
        f"- fidelity-passing fraction: {fidelity_fraction:.1%}; mean local incompat: {mean_local:.3f}",
        f"- min cross-task distance: {manifest['fiber']['min_cross_task_distance']}",
        f"- target barrier hash: `{barrier_hash}`",
    ]
    (out_dir / "branch_adjudication.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    (out_dir / "commands.md").write_text("# Phase 3E v2 invocation\n\n```\n" + " ".join([sys.executable, "docs/prereg/arc/phase3e_program_sketch_oracle_v2.py", *sys.argv[1:]]) + "\n```\n", encoding="utf-8")
    write_json(out_dir / "hashes.json", hash_receipt_files(out_dir))
    print(f"ARC Phase 3E v2 oracle wrote {out_dir}")
