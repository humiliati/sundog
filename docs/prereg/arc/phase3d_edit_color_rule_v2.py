#!/usr/bin/env python
"""ARC Phase 3D edit-color-rule variant runner (structured_edit_color_rule_v2).

Standalone Python runner. Does not import phase3_decoder.py,
phase3a_per_task_coord_mlp.py, or phase3d_structured_edit_residual.py.
The arc-p3-feature-v1 encoders, baseline family, and mask MLP are copied
verbatim from the base Branch D runner (with header markers). The
EditColorMLP is replaced by a deterministic conditioning-derived color-rule
bank (`edit_color_rule_bank_v1`). See PHASE3D_EDIT_COLOR_RULE_VARIANT_SPEC.md
§"Color Rule Bank" for the 10 rule families.

Spec: docs/prereg/arc/PHASE3D_EDIT_COLOR_RULE_VARIANT_SPEC.md (filed 2026-05-28).
...
# ============================================================================
# Frozen Phase 3D variant constants (edit-color-rule bottleneck)
# ============================================================================
FEATURE_SCHEMA_VERSION = "arc-p3-feature-v1"
LEARNER_VERSION = "edit_color_rule_bank_v1"
VARIANT_VERSION = "structured_edit_color_rule_v2"
PROTOCOL_VERSION = "arc-p3d-edit-color-rule-v2"
RECEIPT_SCHEMA_VERSION = "arc-p3d-edit-color-rule-receipt-v1"

ARMS = [
    "raw_grid_edit_color_v2",
    "signature_palette_edit_color_v2",
    "signature_only_edit_color_v2",
    "metadata_only_edit_color_v2",
]
GRID_SCORABLE_ARMS = {"raw_grid_edit_color_v2", "signature_palette_edit_color_v2"}

# Per spec §"Color Rule Bank". Order is the rule-family tiebreak index.
RULE_FAMILIES = [
    "constant_edit_color",
    "modal_edit_color",
    "baseline_color_map",
...
def feature_vector(grid: list[list[int]], arm: str) -> list[float]:
    ...
    if arm == "raw_grid_edit_color_v2":
        return raw_grid_onehot(grid)
    rep = represent_grid(grid, arm)
    vector = [0.0] * SIGNATURE_VECTOR_DIM
    if arm in {"signature_palette_edit_color_v2", "metadata_only_edit_color_v2"}:
        vector[:METADATA_DIM] = rep["metadata"]
    if arm in {"signature_palette_edit_color_v2", "signature_only_edit_color_v2"}:
...
def input_dim_for_arm(arm: str) -> int:
    return RAW_GRID_DIM if arm == "raw_grid_edit_color_v2" else SIGNATURE_VECTOR_DIM
...
def select_color_rule(candidates: list[dict[str, Any]], conditioning: list[dict[str, Any]], cond_baselines: list[list[list[int]]], master_seed: int, lane: str, task_id: str, query_index: int, arm: str) -> dict[str, Any]:
    ...
    for c in candidates:
        ...
        tiebreak_key = hashlib.sha256(
            f"arc-p3d-edit-color-rule-v2\0{master_seed}\0{lane}\0{task_id}\0{query_index}\0{arm}\0{c['family']}|{c['id']}".encode("utf-8")
        ).hexdigest()
...
def assign_quarantine_label(record: dict[str, Any], conditioning_n: int) -> str:
    """Per spec §"Quarantine Labels" — 12 pre-registered labels for the variant.
    Order is the priority for the primary label."""
    if record["grid_exact"]:
        return ""
    # Pre-conditions inherited from Branch D
    ...
    if record["edit_mask_f1"] < 0.50:
        return "edit_mask_failure"
    # Variant-specific color-rule labels
    if record.get("no_conditioning_edits"):
        return "no_conditioning_edits"
    oracle = record.get("color_oracle_rule_accuracy", 0.0)
    selected = record.get("edit_color_rule_accuracy", 0.0)
    if oracle < 0.50:
        return "color_rule_bank_coverage_failure"
    if selected < 0.50:
        return "color_rule_selection_failure"
    # source_binding_failure: rule uses a source-binding family AND the source key
    # is missing or mismatched on target residual cells (rough heuristic: family is
    # a source-binding family AND mask_conditioned_color_accuracy < 0.50)
    source_binding_families = {
        "baseline_color_map", "input_nn_color_map", "input_patch_majority_map",
        "baseline_to_input_pair_map", "object_role_color_map", "nearest_edited_neighbor_color",
    }
    family = record.get("color_rule_family", "")
    if family in source_binding_families and record.get("mask_conditioned_color_accuracy", 1.0) < 0.50:
...
def adjudicate_arena_gate(per_task_rows: list[dict[str, Any]]) -> dict[str, Any]:
    lodo = nonbaseline_exact_task_count(per_task_rows, "test_lodo", "raw_grid_edit_color_v2")
    pt = nonbaseline_exact_task_count(per_task_rows, "pttest", "raw_grid_edit_color_v2")
    if lodo >= 1 and pt >= 1:
        return {
            "gate": "raw_grid_edit_color_v2_arena_open",
            "test_lodo_nonbaseline_exact_tasks": lodo,
            "pttest_nonbaseline_exact_tasks": pt,
        }
    return {
        "gate": "branch_d_color_rule_full_grid_floor",
        "test_lodo_nonbaseline_exact_tasks": lodo,
        "pttest_nonbaseline_exact_tasks": pt,
    }


def adjudicate_branch_d_color_rule(per_task_rows: list[dict[str, Any]], per_lane_rows: list[dict[str, Any]], arena: dict[str, Any]) -> dict[str, Any]:
    if arena["gate"] != "raw_grid_edit_color_v2_arena_open":
        return {
            "branch": "branch_d_color_rule_full_grid_floor",
            "reason": "raw_grid_edit_color_v2 did not open the non-baseline arena (>=1 non-baseline exact task on each held-out lane required)",
        }
    raw_lodo = nonbaseline_exact_task_count(per_task_rows, "test_lodo", "raw_grid_edit_color_v2")
    raw_pt = nonbaseline_exact_task_count(per_task_rows, "pttest", "raw_grid_edit_color_v2")
    sig_lodo = nonbaseline_exact_task_count(per_task_rows, "test_lodo", "signature_palette_edit_color_v2")
    sig_pt = nonbaseline_exact_task_count(per_task_rows, "pttest", "signature_palette_edit_color_v2")
    by_la = {(r["lane"], r["arm"]): r for r in per_lane_rows}
    raw_lodo_r = by_la.get(("test_lodo", "raw_grid_edit_color_v2"), {})
    sig_lodo_r = by_la.get(("test_lodo", "signature_palette_edit_color_v2"), {})
    raw_pt_r = by_la.get(("pttest", "raw_grid_edit_color_v2"), {})
    sig_pt_r = by_la.get(("pttest", "signature_palette_edit_color_v2"), {})
    def fnum(d, k):
        return float(d.get(k) or 0.0)
    # Per spec §"Branch D Variant Adjudication": color_rule_accuracy gap,
    # rare_edit_color_recall gap, rule_selection_regret gap, all ≤ 0.10.
    acc_gap_lodo = fnum(raw_lodo_r, "edit_color_rule_accuracy_mean") - fnum(sig_lodo_r, "edit_color_rule_accuracy_mean")
    acc_gap_pt = fnum(raw_pt_r, "edit_color_rule_accuracy_mean") - fnum(sig_pt_r, "edit_color_rule_accuracy_mean")
    rare_gap_lodo = fnum(raw_lodo_r, "rare_edit_color_recall_mean") - fnum(sig_lodo_r, "rare_edit_color_recall_mean")
    rare_gap_pt = fnum(raw_pt_r, "rare_edit_color_recall_mean") - fnum(sig_pt_r, "rare_edit_color_recall_mean")
    regret_gap_lodo = fnum(sig_lodo_r, "rule_selection_regret_mean") - fnum(raw_lodo_r, "rule_selection_regret_mean")
    regret_gap_pt = fnum(sig_pt_r, "rule_selection_regret_mean") - fnum(raw_pt_r, "rule_selection_regret_mean")
    if (
        sig_lodo >= 1 and sig_pt >= 1
        and (raw_lodo - sig_lodo) <= 1 and (raw_pt - sig_pt) <= 1
        and acc_gap_lodo <= 0.10 and acc_gap_pt <= 0.10
        and rare_gap_lodo <= 0.10 and rare_gap_pt <= 0.10
        and regret_gap_lodo <= 0.10 and regret_gap_pt <= 0.10
    ):
        return {
            "branch": "branch_d_color_rule_support",
            "reason": "signature_palette_edit_color_v2 opens the arena and meets all support thresholds (exact task delta <= 1, color-rule accuracy gap <= 0.10, rare-color recall gap <= 0.10, rule-selection regret gap <= 0.10)",
            "raw_grid_edit_color_v2": {"test_lodo": raw_lodo, "pttest": raw_pt},
            "signature_palette_edit_color_v2": {"test_lodo": sig_lodo, "pttest": sig_pt},
        }
    return {
        "branch": "branch_d_color_rule_bounded_failure",
        "reason": "raw_grid_edit_color_v2 opened the non-baseline arena but signature_palette_edit_color_v2 did not satisfy the support thresholds",
        "raw_grid_edit_color_v2": {"test_lodo": raw_lodo, "pttest": raw_pt},
        "signature_palette_edit_color_v2": {"test_lodo": sig_lodo, "pttest": sig_pt},
    }
...
def assert_shard_consistency(shards: list[dict[str, Any]], repo_root: Path | None = None, allow_mixed_commits: bool = False) -> dict[str, Any] | None:
    ...
    keys = [
        ...
        "maskModelSpec", "shapeRules", "canvasRules", "maskThresholds", "ruleFamilies", "ensembleTieTolerance", "ensembleMinMembers",
        "seedSlate", "arms",
        "maxStepsEffective",
    ]
    ...
    runner_path = "docs/prereg/arc/phase3d_edit_color_rule_v2.py"
...
def run_merge(args) -> int:
    ...
    branch = adjudicate_branch_d_color_rule(per_task_rows_agg, scores, arena)
    ...
    merged_manifest.update({
        ...
        "completedAt": iso_now(),
        "tool": "docs/prereg/arc/phase3d_edit_color_rule_v2.py (merge)",
        "command": [sys.executable, "docs/prereg/arc/phase3d_edit_color_rule_v2.py", *sys.argv[1:]],
        "mode": "full",
        "shardedRun": True,
        "shardSources": [
            {
    ...
    write_json(out_dir / "phase3d_color_rule_receipt.json", {
    ...
    (out_dir / "commands.md").write_text(
        ...
        + " ".join([sys.executable, "docs/prereg/arc/phase3d_edit_color_rule_v2.py", *sys.argv[1:]])
        + "\n```\n"
...
def base_manifest(args, started_at, git, data_dir, register_path, out_dir, register_hash, data_hash, spec_hash, parent_spec_hash, base_branch_d_spec_hash, instance_count) -> dict[str, Any]:
    return {
        "generatedAt": started_at,
        "completedAt": None,
        "tool": "docs/prereg/arc/phase3d_edit_color_rule_v2.py",
        "command": [sys.executable, "docs/prereg/arc/phase3d_edit_color_rule_v2.py", *sys.argv[1:]],
        ...
        "specPath": "docs/prereg/arc/PHASE3D_EDIT_COLOR_RULE_VARIANT_SPEC.md",
        "specHash": spec_hash,
...
PER_TASK_COLS = ["lane", "task_id", "primary_prior", "predicted_boundary", "arm", "selected_seed", "instance_count", "grid_exact_any_rate", "nonbaseline_exact_any_rate", "baseline_exact_any_rate", "shape_exact_rate", "palette_exact_rate", "pixel_accuracy_mean", "edit_color_rule_accuracy_mean", "rare_edit_color_recall_mean", "rule_selection_regret_mean"]
PER_PRIOR_COLS = ["lane", "primary_prior", "arm", "instance_count", "grid_exact_any_rate", "nonbaseline_exact_any_rate", "edit_mask_f1_mean", "edit_color_rule_accuracy_mean"]
...
def write_empty_receipt(out_dir: Path, manifest: dict[str, Any]) -> None:
    ...
    write_json(out_dir / "phase3d_color_rule_receipt.json", {"manifest": manifest, "arenaGate": None, "branchAdjudication": None})
    (out_dir / "branch_adjudication.md").write_text(
...
def main() -> int:
    ...
    spec_path = Path(__file__).resolve().parent / "PHASE3D_EDIT_COLOR_RULE_VARIANT_SPEC.md"
    ...
    manifest = base_manifest(args, started_at, git, data_dir, register_path, out_dir, register_hash, data_hash, spec_hash, parent_spec_hash, base_branch_d_spec_hash, len(all_instances))
    ...
    for arm in arms:
        for seed in seeds:
            for inst in held_out_instances:
                ...
                # 1. Baseline candidate selection on conditioning pairs (inherited unchanged from Phase 3D)
                ...
                # 2. Build mask training rows (color rows not needed — color is rule-bank)
                mask_X, mask_y, _, _ = build_conditioning_examples(arm, inst.conditioning, cond_baselines)
                mask_seed = derive_seed(seed, inst.lane, inst.task_id, inst.query_index, arm, "mask")
                # 3. Fit mask model (inherited unchanged)
                mask_model, mask_info = fit_mask(arm, mask_X, mask_y, mask_seed, max_steps_mask, device)
                # 4. Build candidate color rules from conditioning + cond_baselines
                rule_candidates = generate_candidate_rules(inst.conditioning, cond_baselines)
                # 5. Select a color rule (or ensemble)
                ...
                # 6. Predict conditioning masks + colors for threshold selection
                cond_probs = [predict_mask_probs(mask_model, arm, p["input"], b, device) for p, b in zip(inst.conditioning, cond_baselines)]
                # For threshold selection we use the SELECTED rule to predict full-grid colors,
                # then the threshold sweep treats varying mask thresholds as the search space.
                cond_full_colors = [
                    (
                        predict_query_edit_colors(rule_selection, p["input"], b, [[True] * len(b[0]) for _ in range(len(b))])
                        if rule_selection.get("selected") is not None
                        else [list(row) for row in b]
                    )
                    for p, b in zip(inst.conditioning, cond_baselines)
                ]
                ...
                # 7. Predict query mask + colors at chosen threshold
                ...
                ec_rule_acc = ec_acc  # rule-bank's edit-color accuracy is the same metric
    ...
    if manifest["mode"] == "full":
        ...
        branch = adjudicate_branch_d_color_rule(per_task_rows, scores, arena)
    else:
