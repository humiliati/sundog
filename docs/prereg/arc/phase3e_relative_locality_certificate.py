...
# ============================================================================
# Relative-locality certificate: rank-based locality, controls, nulls, branches
# (spec PHASE3E_RELATIVE_LOCALITY_CERTIFICATE_SPEC.md). The locality metric uses
# ONLY program_sketch_v2 facet Jaccard -- never target hashes, raw outputs, exact
# coordinates, or per-cell masks. exact_lookup / unique_core are diagnostics.
# ============================================================================
import random as _random
import statistics as _stats
from collections import defaultdict as _defaultdict

# Frozen rank cuts + slates (spec §"Neighbor Sets" / §"Controls And Nulls").
K_VALUES = [1, 3, 5, 10]
K_PRIMARY = 5
...
# Frozen branch thresholds (spec §"Branch Criteria"). Not retunable after output.
DELTA_META_MIN = 0.10            # positive: delta_palette_vs_metadata >= 0.10
DELTA_RANDOM_MIN = 0.15          # positive: delta_palette_vs_uniform_random >= 0.15
P_PRIMARY_MAX = 0.01             # positive: p_palette_vs_prior_stratified_random <= 0.01
HARD_INCOMPAT_POS_MAX = 0.10     # positive ceiling on palette hard-incompat rate
HARD_INCOMPAT_NEG_MIN = 0.25     # negative if palette hard-incompat rate > 0.25
DELTA_META_ONLY_MAX = 0.05       # metadata_only if delta_palette_vs_metadata < 0.05


def facet_labels(sketch: dict[str, Any], facet: str) -> set:
    return set(_concrete_labels(sketch["facets"][facet]))


def sketch_sim(sa: dict[str, Any], sb: dict[str, Any]) -> float:
    """Mean per-facet Jaccard over the nine facets, dropping none/unknown. A facet
    is omitted (not scored 0) when BOTH contexts have no concrete label for it."""
    sims = []
    for f in SKETCH_FACETS:
        la, lb = facet_labels(sa, f), facet_labels(sb, f)
        if not la and not lb:
            continue
        union = la | lb
        sims.append((len(la & lb) / len(union)) if union else 0.0)
    return (sum(sims) / len(sims)) if sims else 0.0


def rule_scope_family(sketch: dict[str, Any]) -> tuple:
    return tuple(sorted(facet_labels(sketch, "rule_scope")))


def neighbor_graph(arm: str, primary: list[dict[str, Any]], kmax: int) -> dict[str, list[dict[str, Any]]]:
    """Cross-task nearest neighbors per context under context_distance(arm).
    Same-task neighbors excluded from the primary graph."""
    graph: dict[str, list[dict[str, Any]]] = {}
    for c in primary:
        scored = []
        for o in primary:
            if o["instance_id"] == c["instance_id"] or o["task_id"] == c["task_id"]:
                continue
            scored.append((context_distance(arm, c, o), o["instance_id"], o))
        scored.sort(key=lambda t: (t[0], t[1]))
        graph[c["instance_id"]] = [t[2] for t in scored[:kmax]]
    return graph


def graph_metrics(graph: dict[str, list[dict[str, Any]]], sketches: dict[str, dict], primary: list[dict[str, Any]], k: int) -> dict[str, Any]:
    per_ctx_sim: list[float] = []
    hard_pairs = same_prior = same_family = n_pairs = 0
    by_prior: dict[str, list[float]] = _defaultdict(list)
    for c in primary:
        neigh = graph[c["instance_id"]][:k]
        if not neigh:
            continue
        sa = sketches[c["instance_id"]]
        fam_a = rule_scope_family(sa)
        sims = []
        for o in neigh:
            sb = sketches[o["instance_id"]]
            s = sketch_sim(sa, sb)
            sims.append(s)
            n_pairs += 1
            if _hard_incompat(sa, sb):
                hard_pairs += 1
            if o["primary_prior"] == c["primary_prior"]:
                same_prior += 1
            fam_b = rule_scope_family(sb)
            if fam_a and fam_a == fam_b:
                same_family += 1
        m = sum(sims) / len(sims)
        per_ctx_sim.append(m)
        by_prior[c["primary_prior"]].append(m)
    mean_sim = (sum(per_ctx_sim) / len(per_ctx_sim)) if per_ctx_sim else 0.0
    return {
        "mean_neighbor_sketch_sim": mean_sim,
        "median_neighbor_sketch_sim": (_stats.median(per_ctx_sim) if per_ctx_sim else 0.0),
        "mean_neighbor_sketch_dist": 1.0 - mean_sim,
        "hard_incompatibility_rate": (hard_pairs / n_pairs) if n_pairs else 0.0,
        "same_primary_prior_rate": (same_prior / n_pairs) if n_pairs else 0.0,
        "same_rule_scope_family_rate": (same_family / n_pairs) if n_pairs else 0.0,
        "per_prior": {p: (sum(v) / len(v)) for p, v in by_prior.items()},
        "_per_ctx_sim": per_ctx_sim,
        "n_pairs": n_pairs,
    }


def bootstrap_ci(per_ctx_sim: list[float], seed: int, n_boot: int = N_BOOTSTRAP) -> tuple[float, float]:
    if not per_ctx_sim:
        return (0.0, 0.0)
    ...
    for _ in range(n_boot):
        means.append(sum(per_ctx_sim[rng.randrange(n)] for _ in range(n)) / n)
    means.sort()
    return (means[int(0.025 * n_boot)], means[int(0.975 * n_boot)])


def _precompute_pools(primary: list[dict[str, Any]]) -> tuple[dict, dict]:
    pool: dict[str, list[dict]] = {}
    prior_buckets: dict[str, dict[str, list[dict]]] = {}
    for c in primary:
        cand = [o for o in primary if o["task_id"] != c["task_id"]]
        pool[c["instance_id"]] = cand
        buckets: dict[str, list[dict]] = _defaultdict(list)
        for o in cand:
            buckets[o["primary_prior"]].append(o)
        prior_buckets[c["instance_id"]] = dict(buckets)
    return pool, prior_buckets


def random_control_null(primary, sketches, k, mode, palette_graph, pool, prior_buckets, seeds, n_draws) -> list[float]:
    """mode in {uniform, prior_stratified}. Each draw assigns k random cross-task
    neighbors per context and returns the mean over contexts of mean neighbor
    sketch_sim. prior_stratified matches the prior multiset of the palette kNN."""
    null: list[float] = []
    for seed in seeds:
        rng = _random.Random(seed)
        for _ in range(n_draws):
            ...
            for c in primary:
                ...
                if not cand:
                    continue
                if mode == "prior_stratified":
                    chosen = []
                    for o in palette_graph[cid][:k]:
                        bucket = prior_buckets[cid].get(o["primary_prior"]) or cand
                        chosen.append(bucket[rng.randrange(len(bucket))])
                else:
                    if len(cand) <= k:
                        chosen = cand
                    else:
                        idxs = rng.sample(range(len(cand)), k)
                        chosen = [cand[i] for i in idxs]
                sa = sketches[cid]
                sims = [sketch_sim(sa, sketches[o["instance_id"]]) for o in chosen]
                if sims:
                    per_ctx.append(sum(sims) / len(sims))
            if per_ctx:
                null.append(sum(per_ctx) / len(per_ctx))
    return null


def permutation_null(palette_graph, sketches, primary, k, mode, seeds, n_perm) -> list[float]:
    """mode in {task_label, global}. Shuffle the context->sketch assignment (within
    primary prior, or globally) and recompute mean neighbor sketch_sim on the FROZEN
    palette neighbor graph."""
    ids = [c["instance_id"] for c in primary]
    prior_of = {c["instance_id"]: c["primary_prior"] for c in primary}
    by_prior_ids: dict[str, list[str]] = _defaultdict(list)
    for cid in ids:
        by_prior_ids[prior_of[cid]].append(cid)
    null: list[float] = []
    for seed in seeds:
        ...
        for _ in range(n_perm):
            ...
            if mode == "global":
                ...
                for i, cid in enumerate(ids):
                    smap[cid] = sketches[perm[i]]
            else:  # task_label: shuffle sketch assignment within each primary prior
                for group in by_prior_ids.values():
                    perm = group[:]
                    rng.shuffle(perm)
            ...
            per_ctx = []
            for c in primary:
                neigh = palette_graph[c["instance_id"]][:k]
                if not neigh:
                    continue
                sa = smap[c["instance_id"]]
                sims = [sketch_sim(sa, smap[o["instance_id"]]) for o in neigh]
                per_ctx.append(sum(sims) / len(sims))
            if per_ctx:
                null.append(sum(per_ctx) / len(per_ctx))
    return null
...
def perm_p(observed: float, null: list[float]) -> float:
    ...
    return (1 + ge) / (1 + n) if n else 1.0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ARC Phase 3E relative-locality certificate")
    ...
    parser.add_argument("--permutations", type=int, default=N_PERMUTATIONS, help="per-seed draws for nulls/random controls (frozen binding value 1000)")
    parser.add_argument("--bootstrap", type=int, default=N_BOOTSTRAP)
    parser.add_argument("--seeds", default=",".join(str(s) for s in SEED_SLATE), help="comma-separated RNG seeds (frozen binding slate)")
...
RL_FILES_EMPTY = {
    ...
    "oracle_caveat_audit.csv": ["metric", "value", "threshold", "role", "pass"],
    "neighbor_graphs.csv": ["arm", "k", "context_id", "task_id", "neighbor_context_id", "neighbor_task_id", "rank", "distance", "sketch_sim"],
    "neighbor_similarity_by_context.csv": ["arm", "k", "context_id", "task_id", "primary_prior", "mean_neighbor_sketch_sim"],
    "relative_locality_summary.csv": ["arm", "k", "mean_neighbor_sketch_sim", "median_neighbor_sketch_sim", "mean_neighbor_sketch_dist", "hard_incompatibility_rate", "same_primary_prior_rate", "same_rule_scope_family_rate", "ci_lo", "ci_hi"],
    "per_prior_relative_locality.csv": ["arm", "k", "primary_prior", "mean_neighbor_sketch_sim"],
    "permutation_nulls.csv": ["null", "k", "n_draws", "null_mean", "null_p2_5", "null_p97_5", "observed_palette", "p_value"],
    "bootstrap_intervals.csv": ["arm", "k", "mean_neighbor_sketch_sim", "ci_lo", "ci_hi"],
}


def write_empty_rl_receipt(out_dir: Path, manifest: dict[str, Any]) -> None:
    write_json(out_dir / "manifest.json", manifest)
    for fname, cols in RL_FILES_EMPTY.items():
    ...
    write_jsonl(out_dir / "program_sketch_v2.jsonl", [])
    write_json(out_dir / "phase3e_relative_locality_certificate_receipt.json", {"manifest": manifest, "branch": None})
    (out_dir / "branch_adjudication.md").write_text("# Phase 3E Relative-Locality Certificate\n\nDry run / empty receipt.\n", encoding="utf-8")
    (out_dir / "commands.md").write_text("# Phase 3E relative-locality commands\n\nDry run / empty receipt.\n", encoding="utf-8")
...
def main() -> int:
    ...
    spec_path = Path(__file__).resolve().parent / "PHASE3E_RELATIVE_LOCALITY_CERTIFICATE_SPEC.md"
    parent_specs = [
        "PHASE3E_SIGNATURE_FIBER_CERTIFICATE_SPEC.md",
        "PHASE3E_PROGRAM_SKETCH_ORACLE_V2_SPEC.md",
        "PHASE0_CONTEXT_EXPANSION_FOR_FIBERS_SPEC.md",
    ]
    self_hash = sha256_file(Path(__file__).resolve())

    manifest: dict[str, Any] = {
        "generatedAt": started_at, "completedAt": None,
        "tool": "docs/prereg/arc/phase3e_relative_locality_certificate.py",
        "command": [sys.executable, "docs/prereg/arc/phase3e_relative_locality_certificate.py", *sys.argv[1:]],
        ...
        "specPath": "docs/prereg/arc/PHASE3E_RELATIVE_LOCALITY_CERTIFICATE_SPEC.md",
        "specHash": (sha256_file(spec_path) if spec_path.exists() else "NA"),
        "parentSpecHashes": {p: (sha256_file(spec_path.parent / p) if (spec_path.parent / p).exists() else "NA") for p in parent_specs},
        "runnerSha256": self_hash, "pythonVersion": sys.version, "platform": platform.platform(),
        "arms": CERT_ARMS, "primaryArm": PRIMARY_CERT_ARM,
        "kValues": K_VALUES, "kPrimary": K_PRIMARY, "seeds": seeds,
        "nPermutations": args.permutations, "nBootstrap": args.bootstrap,
        "thresholds": {
            "epsilon_primary": EPSILON_PRIMARY, "epsilon_strict": EPSILON_STRICT, "epsilon_loose": EPSILON_LOOSE,
            "delta_meta_min": DELTA_META_MIN, "delta_random_min": DELTA_RANDOM_MIN,
            "p_primary_max": P_PRIMARY_MAX, "hard_incompat_pos_max": HARD_INCOMPAT_POS_MAX,
            "hard_incompat_neg_min": HARD_INCOMPAT_NEG_MIN, "delta_meta_only_max": DELTA_META_ONLY_MAX,
        },
    }

    if args.dry_run:
        manifest["mode"] = "dry_run"; manifest["completedAt"] = iso_now()
        write_empty_rl_receipt(out_dir, manifest)
        print(f"ARC Phase 3E relative-locality dry run wrote {out_dir}")
        return 0
    ...
    write_csv(out_dir / "split.csv", [{"task_id": t.task_id, "primary_prior": t.primary_prior, "predicted_boundary": t.predicted_boundary, "split": t.split} for t in sorted(tasks, key=lambda x: x.task_id)], RL_FILES_EMPTY["split.csv"])

    def to_ctx(inst: Instance) -> dict[str, Any]:
        return {"lane": inst.lane, "instance_id": inst.instance_id, "task_id": inst.task_id,
                "primary_prior": inst.primary_prior, "predicted_boundary": inst.predicted_boundary,
                "query_index": inst.query_index, "query_input": inst.query_input,
                "target_output": inst.target_output, "conditioning": inst.conditioning}
    ...
    manifest["contextUniverse"] = {"validation_lodo": len(val_lodo), "validation_pttest": len(val_pttest), "test_lodo": len(test_lodo), "pttest": len(test_pttest), "u_all": len(u_all), "u_primary": len(u_primary)}

    # Stage 1: no-target fingerprints + barrier
    fingerprints = [{
        "instance_id": c["instance_id"], "lane": c["lane"], "task_id": c["task_id"], "query_index": c["query_index"],
        "in_u_primary": c in u_primary,
    ...
    # Stage 2: program_sketch_v2 for every context (after the barrier)
    sketches: dict[str, dict[str, Any]] = {c["instance_id"]: program_sketch_v2(c) for c in u_all}
    write_jsonl(out_dir / "program_sketch_v2.jsonl", [{"instance_id": cid, **sketches[cid]} for cid in sorted(sketches)])

    # ---- Oracle caveat audit (syntactic fatal; vacuity/laundering gate; exact_lookup/unique_core diagnostic) ----
    syntactic_fails = []
    for c in u_all:
        syntactic_fails += _syntactic_leakage(sketches[c["instance_id"]])
    primary_sketches = [sketches[c["instance_id"]] for c in u_primary]
    vacuous_fraction = (sum(1 for s in primary_sketches if is_vacuous(s)) / len(primary_sketches)) if primary_sketches else 0.0
    by_prior_vac: dict[str, list[int]] = _defaultdict(list)
    for c in u_primary:
        by_prior_vac[c["primary_prior"]].append(1 if is_vacuous(sketches[c["instance_id"]]) else 0)
    per_prior_vac_max = max((sum(v) / len(v)) for v in by_prior_vac.values()) if by_prior_vac else 0.0
    # Anti-prior-laundering, defined exactly as the v2 gate: a NON-VACUOUS context
    # violates when its extra-facet count is below LAUNDERING_MIN_EXTRA_FACETS; the
    # fraction is over non-vacuous primary contexts.
    n_nonvacuous = sum(1 for s in primary_sketches if not is_vacuous(s))
    n_laundering_violations = sum(
        1 for c in u_primary
        if (not is_vacuous(sketches[c["instance_id"]]))
        and (laundering_extra_facet_count(sketches[c["instance_id"]], c["primary_prior"]) < LAUNDERING_MIN_EXTRA_FACETS)
    )
    laundering_violation_fraction = (n_laundering_violations / n_nonvacuous) if n_nonvacuous else 0.0
    vacuity_fail = (vacuous_fraction > VACUITY_OVERALL_MAX) or (per_prior_vac_max > VACUITY_PER_PRIOR_MAX)
    laundering_fail = laundering_violation_fraction > LAUNDERING_MAX_VIOLATION_FRACTION
    # diagnostic: exact_lookup / unique_core over U_primary (caveat, NOT a fail gate)
    for c in u_primary:
        c["_target_exact_hash"] = target_exact_hash(c["target_output"]) if c.get("target_output") is not None else None
    core_tuples = [core_sketch_tuple(sketches[c["instance_id"]]) for c in u_primary]
    unique_core_fraction = (len(set(core_tuples)) / len(core_tuples)) if core_tuples else 0.0
    core_groups: dict[tuple, list[dict]] = _defaultdict(list)
    for c in u_primary:
        core_groups[core_sketch_tuple(sketches[c["instance_id"]])].append(c)
    solvable = 0
    for grp in core_groups.values():
        if len({m["_target_exact_hash"] for m in grp}) == 1:
            solvable += len(grp)
    exact_lookup_fraction = (solvable / len(u_primary)) if u_primary else 0.0
    oracle_invalid = bool(syntactic_fails) or vacuity_fail or laundering_fail
    write_csv(out_dir / "oracle_caveat_audit.csv", [
        {"metric": "syntactic_leakage_failures", "value": ";".join(sorted(set(syntactic_fails))) or "none", "threshold": "0", "role": "fatal_gate", "pass": not syntactic_fails},
        {"metric": "vacuous_fraction", "value": round_float(vacuous_fraction), "threshold": VACUITY_OVERALL_MAX, "role": "gate", "pass": not vacuity_fail},
        {"metric": "per_prior_vacuous_max", "value": round_float(per_prior_vac_max), "threshold": VACUITY_PER_PRIOR_MAX, "role": "gate", "pass": per_prior_vac_max <= VACUITY_PER_PRIOR_MAX},
        {"metric": "laundering_violation_fraction", "value": round_float(laundering_violation_fraction), "threshold": LAUNDERING_MAX_VIOLATION_FRACTION, "role": "gate", "pass": not laundering_fail},
        {"metric": "core_sketch_exact_lookup_fraction", "value": round_float(exact_lookup_fraction), "threshold": LEAKAGE_EXACT_LOOKUP_MAX, "role": "diagnostic_caveat", "pass": "n/a"},
        {"metric": "unique_core_sketch_fraction", "value": round_float(unique_core_fraction), "threshold": LEAKAGE_UNIQUE_CORE_MAX, "role": "diagnostic_caveat", "pass": "n/a"},
    ], RL_FILES_EMPTY["oracle_caveat_audit.csv"])

    # ---- Neighbor graphs per arm (kmax = max K) ----
    kmax = max(K_VALUES)
    graphs = {arm: neighbor_graph(arm, u_primary, kmax) for arm in CERT_ARMS}
    palette_graph = graphs[PRIMARY_CERT_ARM]

    # neighbor_graphs.csv (palette arm, k=kmax, for audit) + similarity-by-context (all arms, k_primary)
    ng_rows = []
    for arm in CERT_ARMS:
        for c in u_primary:
            for rank, o in enumerate(graphs[arm][c["instance_id"]][:kmax], start=1):
                ng_rows.append({"arm": arm, "k": kmax, "context_id": c["instance_id"], "task_id": c["task_id"],
                                "neighbor_context_id": o["instance_id"], "neighbor_task_id": o["task_id"], "rank": rank,
                                "distance": round_float(context_distance(arm, c, o)),
                                "sketch_sim": round_float(sketch_sim(sketches[c["instance_id"]], sketches[o["instance_id"]]))})
    write_csv(out_dir / "neighbor_graphs.csv", ng_rows, RL_FILES_EMPTY["neighbor_graphs.csv"])

    # ---- Metrics per arm and k ----
    ...
    for arm in CERT_ARMS:
        for k in K_VALUES:
            m = graph_metrics(graphs[arm], sketches, u_primary, k)
            metrics[(arm, k)] = m
            ci = bootstrap_ci(m["_per_ctx_sim"], seed=seeds[0], n_boot=args.bootstrap)
            summary_rows.append({"arm": arm, "k": k,
                                 "mean_neighbor_sketch_sim": round_float(m["mean_neighbor_sketch_sim"]),
                                 "median_neighbor_sketch_sim": round_float(m["median_neighbor_sketch_sim"]),
                                 "mean_neighbor_sketch_dist": round_float(m["mean_neighbor_sketch_dist"]),
                                 "hard_incompatibility_rate": round_float(m["hard_incompatibility_rate"]),
                                 "same_primary_prior_rate": round_float(m["same_primary_prior_rate"]),
                                 "same_rule_scope_family_rate": round_float(m["same_rule_scope_family_rate"]),
                                 "ci_lo": round_float(ci[0]), "ci_hi": round_float(ci[1])})
            boot_rows.append({"arm": arm, "k": k, "mean_neighbor_sketch_sim": round_float(m["mean_neighbor_sketch_sim"]), "ci_lo": round_float(ci[0]), "ci_hi": round_float(ci[1])})
            for p, v in sorted(m["per_prior"].items()):
                per_prior_rows.append({"arm": arm, "k": k, "primary_prior": p, "mean_neighbor_sketch_sim": round_float(v)})
            if k == K_PRIMARY:
                for c in u_primary:
                    neigh = graphs[arm][c["instance_id"]][:k]
                    if not neigh:
                        continue
                    sa = sketches[c["instance_id"]]
                    sim = sum(sketch_sim(sa, sketches[o["instance_id"]]) for o in neigh) / len(neigh)
                    simctx_rows.append({"arm": arm, "k": k, "context_id": c["instance_id"], "task_id": c["task_id"], "primary_prior": c["primary_prior"], "mean_neighbor_sketch_sim": round_float(sim)})
    write_csv(out_dir / "relative_locality_summary.csv", summary_rows, RL_FILES_EMPTY["relative_locality_summary.csv"])
    write_csv(out_dir / "per_prior_relative_locality.csv", per_prior_rows, RL_FILES_EMPTY["per_prior_relative_locality.csv"])
    write_csv(out_dir / "bootstrap_intervals.csv", boot_rows, RL_FILES_EMPTY["bootstrap_intervals.csv"])
    write_csv(out_dir / "neighbor_similarity_by_context.csv", simctx_rows, RL_FILES_EMPTY["neighbor_similarity_by_context.csv"])

    # ---- Controls + nulls (at k_primary and the stability cuts) ----
    pool, prior_buckets = _precompute_pools(u_primary)
    null_rows = []
    null_cache: dict[tuple, list[float]] = {}
    for k in (3, K_PRIMARY, 10):
        obs_palette = metrics[(PRIMARY_CERT_ARM, k)]["mean_neighbor_sketch_sim"]
        uni = random_control_null(u_primary, sketches, k, "uniform", palette_graph, pool, prior_buckets, seeds, args.permutations)
        strat = random_control_null(u_primary, sketches, k, "prior_stratified", palette_graph, pool, prior_buckets, seeds, args.permutations)
        tperm = permutation_null(palette_graph, sketches, u_primary, k, "task_label", seeds, args.permutations)
        gperm = permutation_null(palette_graph, sketches, u_primary, k, "global", seeds, args.permutations)
        null_cache[("uniform", k)] = uni
        null_cache[("prior_stratified", k)] = strat
        for name, dist in [("uniform_random", uni), ("prior_stratified_random", strat), ("task_label_permutation", tperm), ("global_sketch_permutation", gperm)]:
            ds = sorted(dist)
            lo = ds[int(0.025 * len(ds))] if ds else 0.0
            hi = ds[int(0.975 * len(ds))] if ds else 0.0
            null_rows.append({"null": name, "k": k, "n_draws": len(dist),
                              "null_mean": round_float(sum(dist) / len(dist) if dist else 0.0),
                              "null_p2_5": round_float(lo), "null_p97_5": round_float(hi),
                              "observed_palette": round_float(obs_palette), "p_value": round_float(perm_p(obs_palette, dist))})
    write_csv(out_dir / "permutation_nulls.csv", null_rows, RL_FILES_EMPTY["permutation_nulls.csv"])

    # ---- Primary + secondary effects ----
    def mean_sim(arm, k):
        return metrics[(arm, k)]["mean_neighbor_sketch_sim"]

    def uni_mean(k):
        d = null_cache[("uniform", k)]
        return (sum(d) / len(d)) if d else 0.0

    delta_meta = {k: mean_sim(PRIMARY_CERT_ARM, k) - mean_sim("metadata_only_context", k) for k in (3, K_PRIMARY, 10)}
    delta_uni = {k: mean_sim(PRIMARY_CERT_ARM, k) - uni_mean(k) for k in (3, K_PRIMARY, 10)}
    delta_sig_only = mean_sim(PRIMARY_CERT_ARM, K_PRIMARY) - mean_sim("signature_only_context", K_PRIMARY)
    delta_raw = mean_sim(PRIMARY_CERT_ARM, K_PRIMARY) - mean_sim("raw_grid_context", K_PRIMARY)
    p_primary = perm_p(mean_sim(PRIMARY_CERT_ARM, K_PRIMARY), null_cache[("prior_stratified", K_PRIMARY)])
    palette_hard = metrics[(PRIMARY_CERT_ARM, K_PRIMARY)]["hard_incompatibility_rate"]

    # ---- Branch adjudication (table precedence) ----
    stable_sign = (delta_meta[3] > 0 and delta_meta[10] > 0 and delta_uni[3] > 0 and delta_uni[10] > 0)
    positive = (delta_meta[K_PRIMARY] >= DELTA_META_MIN and delta_uni[K_PRIMARY] >= DELTA_RANDOM_MIN and
                p_primary <= P_PRIMARY_MAX and palette_hard <= HARD_INCOMPAT_POS_MAX and stable_sign)
    metadata_only = (delta_uni[K_PRIMARY] >= DELTA_RANDOM_MIN and p_primary <= P_PRIMARY_MAX and
                     delta_meta[K_PRIMARY] < DELTA_META_ONLY_MAX)
    negative = (p_primary > P_PRIMARY_MAX) or (palette_hard > HARD_INCOMPAT_NEG_MIN)

    if oracle_invalid:
        branch = "phase3e_relative_oracle_invalid"
        reason = "Oracle invalid: " + ("syntactic leakage; " if syntactic_fails else "") + (f"vacuity fail ({vacuous_fraction:.2f}); " if vacuity_fail else "") + (f"laundering fail ({laundering_violation_fraction:.2f}); " if laundering_fail else "")
    elif positive:
        branch = "phase3e_relative_locality_positive"
        reason = f"k=5 delta_vs_metadata {delta_meta[K_PRIMARY]:.3f}>={DELTA_META_MIN}, delta_vs_uniform {delta_uni[K_PRIMARY]:.3f}>={DELTA_RANDOM_MIN}, p {p_primary:.4f}<={P_PRIMARY_MAX}, hard_incompat {palette_hard:.3f}<={HARD_INCOMPAT_POS_MAX}, stable across k=3,10."
    elif metadata_only:
        branch = "phase3e_relative_locality_metadata_only"
        reason = f"Positive vs random (delta_vs_uniform {delta_uni[K_PRIMARY]:.3f}, p {p_primary:.4f}) but delta_vs_metadata {delta_meta[K_PRIMARY]:.3f}<{DELTA_META_ONLY_MAX}: coarse metadata explains the locality."
    elif negative:
        branch = "phase3e_relative_locality_negative"
        reason = f"Not above prior-stratified random at k=5 (p {p_primary:.4f}) or hard_incompat {palette_hard:.3f}>{HARD_INCOMPAT_NEG_MIN}: no usable rank-local sketch geometry."
    else:
        branch = "phase3e_relative_locality_inconclusive"
        reason = f"k=5 deltas (meta {delta_meta[K_PRIMARY]:.3f}, uniform {delta_uni[K_PRIMARY]:.3f}, p {p_primary:.4f}) do not meet positive thresholds and are not clearly negative/metadata-only; or not stable across k=3,10 (stable={stable_sign})."

    manifest["completedAt"] = iso_now()
    manifest["oracleCaveat"] = {"syntactic_leakage": bool(syntactic_fails), "vacuous_fraction": round_float(vacuous_fraction),
                                 "laundering_violation_fraction": round_float(laundering_violation_fraction),
                                 "core_sketch_exact_lookup_fraction": round_float(exact_lookup_fraction),
                                 "unique_core_sketch_fraction": round_float(unique_core_fraction), "oracle_invalid": oracle_invalid}
    manifest["effects"] = {
        "delta_palette_vs_metadata_k5": round_float(delta_meta[K_PRIMARY]),
        "delta_palette_vs_uniform_random_k5": round_float(delta_uni[K_PRIMARY]),
        "delta_palette_vs_signature_only_k5": round_float(delta_sig_only),
        "delta_palette_vs_raw_grid_k5": round_float(delta_raw),
        "p_palette_vs_prior_stratified_random_k5": round_float(p_primary),
        "palette_hard_incompatibility_rate_k5": round_float(palette_hard),
        "delta_palette_vs_metadata_k3": round_float(delta_meta[3]),
        "delta_palette_vs_metadata_k10": round_float(delta_meta[10]),
        "stable_sign_k3_k10": stable_sign,
        "palette_mean_sketch_sim_k5": round_float(mean_sim(PRIMARY_CERT_ARM, K_PRIMARY)),
    }
    manifest["branch"] = branch
    write_json(out_dir / "manifest.json", manifest)

    receipt = {"manifest": manifest, "branch": branch, "branchReason": reason}
    write_json(out_dir / "phase3e_relative_locality_certificate_receipt.json", receipt)

    (out_dir / "branch_adjudication.md").write_text(
        f"# Phase 3E Relative-Locality Certificate — Branch Adjudication\n\n"
        f"**Branch: `{branch}`**\n\n{reason}\n\n"
        f"- U_primary: {len(u_primary)} (U_all {len(u_all)}); split_mode {args.split_mode}; seeds {seeds}; permutations {args.permutations}\n"
        f"- palette mean neighbor sketch_sim @k=5: {mean_sim(PRIMARY_CERT_ARM, K_PRIMARY):.4f}\n"
        f"- delta vs metadata @k5: {delta_meta[K_PRIMARY]:.4f} (k3 {delta_meta[3]:.4f}, k10 {delta_meta[10]:.4f})\n"
        f"- delta vs uniform-random @k5: {delta_uni[K_PRIMARY]:.4f}\n"
        f"- delta vs signature_only @k5: {delta_sig_only:.4f}; vs raw_grid @k5: {delta_raw:.4f}\n"
        f"- p vs prior-stratified random @k5: {p_primary:.4f}\n"
        f"- palette hard-incompatibility @k5: {palette_hard:.4f}\n"
        f"- oracle caveat: exact_lookup {exact_lookup_fraction:.3f}, unique_core {unique_core_fraction:.3f}, "
        f"vacuity {vacuous_fraction:.3f}, laundering {laundering_violation_fraction:.3f}, oracle_invalid {oracle_invalid}\n\n"
        f"The absolute-epsilon receipts remain binding; a relative-locality result does not populate fixed-radius fibers.\n",
        encoding="utf-8")
    (out_dir / "commands.md").write_text(
        "# Phase 3E relative-locality commands\n\n```\n"
    ...
    print(f"ARC Phase 3E relative-locality certificate wrote {out_dir}")
