...
| Tangent-arc curvature → h | **fails under three literature-standard detectors** *(Pass C1 + C2 + C4 landed 2026-05-14)* | **tooling protocol.** Pass C1 dropped p7 (h = 59.4° is circumscribed-halo regime). Pass C2 built a wing-radial Lab b\* ridge detector with 22°-halo-radial-profile subtraction and ran it on p2 / p13 / p27: coherent ridge samples 0-2 / ~24 per wing on every photo, below 8 / 24 gate. Pass C4 then built the gradient-based edge detector Persona 1 §5 explicitly named (wing-slope luminance-gradient curvature with circle fit): 88-100% of wing samples rejected as weak-gradient on every photo. Route **not-recovered** under three detector families across two signal modalities (chromatic ridge; luminance edge). The remaining open question narrows to manual sample selection, matched-filter, polarization filtering, or new calibration photos — Phase 10 backlog. |
