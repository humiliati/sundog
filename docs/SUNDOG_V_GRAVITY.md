...
| Tangent-arc curvature → h | **fails under two literature-standard detectors** *(Pass C1 + Pass C2 landed 2026-05-14)* | **tooling protocol.** Pass C1 dropped p7 (h = 59.4° is circumscribed-halo regime). Pass C2 built a wing-radial Lab b\* ridge detector with 22°-halo-radial-profile subtraction and ran it on p2 / p13 / p27: coherent ridge samples 0-2 / ~24 per wing on every photo, well below the pre-registered 8 / 24 gate. Route **not-recovered** under the second literature-standard detector. The remaining open question narrows to non-literature-standard detector designs (wing-slope curvature, matched-filter, polarization), filed as Phase 10 backlog. |
