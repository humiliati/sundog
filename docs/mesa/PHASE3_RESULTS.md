...
- **Medium-tier slate.** Phase 2 Medium PPO has not yet trained. Once
  Medium policies exist, the same probe slate runs unchanged. Expected
  to sharpen the L-Mixed-geometric-light recovery signal if it is real.
- **Larger seed slate.** 64 seeds is enough to surface the program
  pattern but noisy at the +2/+3 success-count level. 256 seeds at
  Medium would let the L-Mixed-recovery prediction be tested
  statistically.
