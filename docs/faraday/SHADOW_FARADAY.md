...
Status: 2026-05-26, Phases 1-6 local readiness signed off, Phase 6b bridge
registered, and Phase 7 boundary-audit spec opened. Phase 3 landed Branch A
...
```text
docs/faraday/FARADAY_PHASE3_DERIVATIONS.md
```
...
external Phase 6 action is a post-deploy LinkedIn/Twitter validator pass.

## Phase 6b: Safety-Method Bridge Note

Registered 2026-05-26.

The Faraday public page now threads onward to `/safety-method`, giving the
site a coherent path:

```text
index.html -> /faraday -> /safety-method
```

This is a bridge registration, not a new Faraday result. The bridge essay is
explicitly marked **SKETCH** and uses the closed Faraday Branch A receipt as a
worked example for three AI-safety method translations:

1. local gauge-invariant readout;
2. structural zero rather than tolerance threshold;
3. named quarantine before post-hoc explanation.

The bridge keeps three load-bearing boundaries:

- Faraday Branch A remains scoped to the registered classical-vacuum,
  magnetically clean, contractible domain.
- `/safety-method` does not prove AI safety, does not prove interpretability,
  and does not turn the Faraday receipt into an AI theorem.
- Good-faith compander / mechanism citation hooks are staged but remain gated
  on a public citation or explicit go-ahead before named credit is pulled into
  the citation rail.

Local site readiness for the bridge is present: bespoke `1200x630`
`og:image`, OG/Twitter metadata, JSON-LD `TechArticle`, sitemap coverage,
clean-url redirect, and the Faraday-thread inbound path. Post-deploy
validators and the future citation ratchet still gate any external push.

Phase 6b exits **registered**. It adds connective tissue downstream of the
Faraday receipt and does not alter the Phase 3/5 Branch A claim.

## Phase 7: Source / Topology Boundary Audit

Opened 2026-05-26. Spec:
[`FARADAY_PHASE7_SPEC.md`](FARADAY_PHASE7_SPEC.md).

Phase 7 is the first post-close extension. It is a boundary audit, not a
claim-broadening pass. The phase asks which nearby domains preserve the
Faraday residual zero and which ones trip the already-registered Branch B
quarantine hooks.

The correction that governs the phase:

- ordinary electric charge/current sources enter `d * F = J` and do not by
  themselves break `dF = 0`;
- magnetic charge/current, singular strings, and non-contractible topology are
  the relevant Faraday boundary cases;
- Ampere-Maxwell / Gauss-law shadow claims are deferred and must not be
  silently folded into this Faraday extension.

Registered Phase 7 cases:

| Case | Domain condition | Expected landing |
| --- | --- | --- |
| Electric-source, magnetically clean control | `J_mu != 0`, `dF = 0` | Faraday residual remains `0`; source-tolerant only in the narrow Bianchi sense. |
| Magnetic-source quarantine | `dF = K_m != 0` | Branch B magnetic-source residual equal to the registered source flux. |
| Topological quarantine | non-contractible solenoid / AB-style loop | Branch B named topology survivor. |

Phase 7 must not use sourced or topological examples to broaden the closed
...
- 2026-05-26 - Phase 6b bridge note registered: `/faraday` now threads to the
  SKETCH `/safety-method` bridge essay, with explicit claim boundaries and
  good-faith citation hooks. This does not change the Faraday Branch A claim.
