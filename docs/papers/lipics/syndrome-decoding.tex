...
% NOTE: the thm-restate class option is omitted -- this paper declares no theorem
% environments, and under TeX Live 2026 that option's thmtools/cleveref counter
% aliasing breaks lipics-v2021.cls v3.1.3 ("\c@lemma already defined"). Re-add it
% only if restatable theorems are introduced (and re-test the build).
...
% \doi is normally supplied by the plainurl BibTeX flow; the bibliography here is
% an inline thebibliography, so provide it (hyperref is loaded by the class).
...
\begin{lstlisting}
theorem litTipFree_iff_eval (a : Assignment n) (l : Literal n)
    {m} [NeZero m] (j : Fin m) :
...
variable-internal nodes, clause-internal nodes, and garbage. Each has cardinality $2mn$; the garbage count $m(n{-}1)$ is exactly what balances the
...
\emph{poly-time} reductions. The Coq
