...
animation use — is the [HaloSim Cinematic Sidecar](#halosim-cinematic-sidecar-hs-0--hs-7)
near the end of this document. Same binary, opposite purpose; do not
conflate the two when scheduling work.
...
## HaloSim Cinematic Sidecar (HS-0 … HS-7)

*Filed 2026-05-14. This is a **sidecar**, not a main-ladder phase: it does
not gate hero promotion and is not in the Phase 0–12 dependency chain. It
is the **generative / cinematic** use of HaloSim, deliberately distinct
from Phase 12A/12B, which use HaloSim as a **validation oracle**. Same
binary, opposite direction: 12 asks "is the atlas right?"; the sidecar
asks "can HaloSim render a beautiful, labelled, sun-altitude-swept halo
film the atlas can narrate?" Outputs feed Phase 7 (press / social
snapshot assets), Phase 11 (characterized animation toolkit), and Phase 4
(active-reveal polish) — it does not replace any of them.*
